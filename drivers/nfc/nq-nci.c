...
static struct of_device_id msm_match_table[] = {
    {.compatible = "nxp,pn547"}, {.compatible = "qcom,nq-nci"}, {}};
...
static struct notifier_block nfcc_notifier = {
    .notifier_call = nfcc_reboot, .next = NULL, .priority = 0};

static unsigned int disable_ctrl;

static void nqx_init_stat(struct nqx_dev *nqx_dev) {
  nqx_dev->count_irq = 0;
  nqx_dev->cancel_wait = false;
}

/* cityman has no eSE. Stock nq-nci only dropped VEN when ese_gpio
 * was valid and low, so NFC_SET_PWR 0 was a no-op and a wedged
 * PN547 could never be reset from userspace.
 */
static void nqx_ven_off(struct nqx_dev *nqx_dev) {
  if (gpio_is_valid(nqx_dev->ese_gpio) && gpio_get_value(nqx_dev->ese_gpio))
    return;
  gpio_set_value(nqx_dev->en_gpio, 0);
}

static void nqx_ven_pulse(struct nqx_dev *nqx_dev) {
  nqx_ven_off(nqx_dev);
  msleep(50);
...
static ssize_t nfc_read(struct file *filp, char __user *buf, size_t count,
  ...
  if (irq_gpio_val == 0) {
    ...
    /* Do not hold read_mutex across the wait: ioctl/write/exit
     * would otherwise sleep uninterruptibly and freeze the phone.
     */
...
#ifdef NFC_KERNEL_BU
  dev_dbg(&nqx_dev->client->dev, "%s : i2c-%d: NfcNciTx %x %x %x\n", __func__,
...
static int nfc_open(struct inode *inode, struct file *filp) {
  ...
  struct nqx_dev *nqx_dev =
      container_of(filp->private_data, struct nqx_dev, nqx_device);

  filp->private_data = nqx_dev;
  nqx_init_stat(nqx_dev);

  dev_dbg(&nqx_dev->client->dev, "%s: %d,%d\n", __func__, imajor(inode),
          iminor(inode));
  return ret;
}
...
static int nfc_release(struct inode *inode, struct file *filp) {
  ...
  if (nqx_dev) {
    nqx_dev->cancel_wait = true;
    wake_up_interruptible(&nqx_dev->read_wq);
  }
  return 0;
}
...
static int nfc_ioctl_power_states(struct file *filp, unsigned long arg) {
  ...
  if (arg == 0) {
    ...
    nqx_dev->cancel_wait = true;
    wake_up_interruptible(&nqx_dev->read_wq);
    dev_dbg(&nqx_dev->client->dev, "gpio_set_value disable: %s: info: %p\n",
            __func__, nqx_dev);
    ...
    nqx_dev->cancel_wait = false;
    dev_dbg(&nqx_dev->client->dev, "gpio_set_value enable: %s: info: %p\n",
            __func__, nqx_dev);
    if (gpio_is_valid(nqx_dev->firm_gpio))
      gpio_set_value(nqx_dev->firm_gpio, 0);
    /* Pulse VEN so a hung PN547 actually resets. */
    nqx_ven_pulse(nqx_dev);
    nqx_enable_irq(nqx_dev);
...
static const struct file_operations nfc_dev_fops = {.owner = THIS_MODULE,
                                                    .llseek = no_llseek,
                                                    .read = nfc_read,
                                                    .write = nfc_write,
                                                    .open = nfc_open,
...
static int nfcc_hw_check(struct i2c_client *client, unsigned int enable_gpio) {
  ...
  ret = i2c_master_send(client, raw_nci_reset_cmd, sizeof(raw_nci_reset_cmd));
  if (ret < 0) {
  ...
  ret = i2c_master_recv(client, nci_reset_rsp, sizeof(nci_reset_rsp));
  dev_err(&client->dev, "%s: - nq - reset cmd answer : NfcNciRx %x %x %x\n",
          __func__, nci_reset_rsp[0], nci_reset_rsp[1], nci_reset_rsp[2]);
  if (ret < 0) {
...
static int nfc_parse_dt(struct device *dev, struct nqx_platform_data *pdata) {
  ...
  if (!gpio_is_valid(pdata->en_gpio))
    pdata->en_gpio = of_get_named_gpio(np, "nxp,gpio_ven", 0);
  if (!gpio_is_valid(pdata->en_gpio))
    return -EINVAL;
  ...
  if (!gpio_is_valid(pdata->irq_gpio))
    pdata->irq_gpio = of_get_named_gpio(np, "nxp,gpio_irq", 0);
  if (!gpio_is_valid(pdata->irq_gpio))
    return -EINVAL;

  pdata->firm_gpio = of_get_named_gpio(np, "qcom,nq-firm", 0);
  if (!gpio_is_valid(pdata->firm_gpio))
    pdata->firm_gpio = of_get_named_gpio(np, "nxp,gpio_mode", 0);
  if (!gpio_is_valid(pdata->firm_gpio)) {
...
static int nqx_probe(struct i2c_client *client,
  ...
  if (client->dev.of_node) {
    platform_data = devm_kzalloc(&client->dev, sizeof(struct nqx_platform_data),
                                 GFP_KERNEL);
    if (!platform_data) {
      r = -ENOMEM;
  ...
  if (!nqx_dev->kbuf) {
    dev_err(&client->dev, "failed to allocate memory for nqx_dev->kbuf\n");
    r = -ENOMEM;
    goto err_free_dev;
  }
  ...
  if (gpio_is_valid(platform_data->en_gpio)) {
    ...
    if (r) {
      dev_err(&client->dev, "%s: unable to request nfc reset gpio [%d]\n",
  ...
  if (gpio_is_valid(platform_data->firm_gpio)) {
    ...
    if (r) {
      dev_err(&client->dev, "%s: unable to request nfc firmware gpio [%d]\n",
    ...
    dev_warn(&client->dev, "%s: firm gpio not provided\n", __func__);
  }
  if (gpio_is_valid(platform_data->ese_gpio)) {
    r = gpio_request(platform_data->ese_gpio, "nfc-ese_pwr");
    if (r) {
      dev_err(&client->dev, "%s: unable to request nfc ese gpio [%d]\n",
    ...
    if (r) {
      dev_err(&client->dev, "%s: cannot set direction for nfc ese gpio [%d]\n",
              __func__, platform_data->ese_gpio);
      /* ese gpio optional so we should continue */
    }
  } else {
  ...
  if (gpio_is_valid(platform_data->clkreq_gpio)) {
    ...
    if (r) {
      dev_err(&client->dev, "%s: unable to request nfc clkreq gpio [%d]\n",
    ...
    dev_warn(&client->dev, "%s: clkreq gpio not provided\n", __func__);
  }
  ...
  /* NFC_INT IRQ */
  nqx_dev->irq_enabled = true;
  r = request_irq(client->irq, nqx_dev_irq_handler, IRQF_TRIGGER_HIGH,
                  client->name, nqx_dev);
  if (r) {
  ...
  if (r) {
    dev_warn(&client->dev, "%s: hw_check failed, keeping /dev/pn547\n",
             __func__);
  ...
  if (r) {
    dev_err(&client->dev, "%s: cannot register reboot notifier(err = %d)\n",
            __func__, r);
  ...
  r = nqx_clock_select(nqx_dev);
  if (r < 0)
    dev_warn(&client->dev, "%s: nqx_clock_select failed, continuing\n",
             __func__);
#ifdef NFC_KERNEL_BU
  ...
  /* Default I2C timeout is long enough for a hung PN547 to
   * watchdog the phone. 250 ms is enough for NCI frames.
   */
  if (client->adapter)
    client->adapter->timeout = msecs_to_jiffies(250);
...
static const struct i2c_device_id nqx_id[] = {{"nqx-i2c", 0}, {}};
 ...
 */
static int __init nqx_dev_init(void) { return i2c_add_driver(&nqx); }
