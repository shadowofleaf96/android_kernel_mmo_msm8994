				...
				struct device_attribute *attr, char *buf)
{
	...
	dev_err(d->dev, "%s: res_ready_show enter\n", __func__);
	...
	dev_err(d->dev, "%s: res_ready_show exit\n", __func__);
				...
				const char *buf, size_t size)
{
	...
	if (!strncmp(buf, init_deinit_cmds[PN547_INIT], PAGE_SIZE)) {
		...
		dev_err(d->dev, "%s: PN547_DEINIT\n", __func__);
		if (atomic_read(&d->res_ready))
			dev_err(d->dev, "%s: interruption is ignored\n",
				__func__);
		atomic_set(&d->res_ready, 0);
		d->busy = false;
	}
