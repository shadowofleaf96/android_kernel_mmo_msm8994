...
static struct firmware_buf *__fw_lookup_buf(const char *fw_name) {
  ...
  list_for_each_entry(tmp, &fwc->head,
                      list) if (!strcmp(tmp->fw_id, fw_name)) return tmp;
  return NULL;
}
...
static bool fw_read_file_contents(struct file *file,
  ...
  if (fw_buf->dest_addr)
    buf = fw_buf->map_fw_mem(fw_buf->dest_addr, fw_buf->dest_size,
                             fw_buf->map_data);
  else
    buf = vmalloc(size);
  if (!buf)
  ...
  if (kernel_read(file, 0, buf, size) != size) {
    if (fw_buf->dest_addr)
      fw_buf->unmap_fw_mem(buf, fw_buf->dest_size, fw_buf->map_data);
    else
      vfree(buf);
    return false;
  }
...
static int fw_devm_match(struct device *dev, void *res, void *match_data) {
  struct fw_name_devm *fwn = res;
...
static struct fw_name_devm *fw_find_devm_name(struct device *dev,
  ...
  fwn = devres_find(dev, fw_name_devm_release, fw_devm_match, (void *)name);
  return fwn;
}
...
static int fw_add_devm_name(struct device *dev, const char *name) {
  ...
  fwn =
      devres_alloc(fw_name_devm_release,
                   sizeof(struct fw_name_devm) + strlen(name) + 1, GFP_KERNEL);
  if (!fwn)
    return -ENOMEM;
...
#define is_fw_load_aborted(buf) test_bit(FW_STATUS_ABORT, &(buf)->status)
...
static struct class_attribute firmware_class_attrs[] = {
    __ATTR(timeout, S_IWUSR | S_IRUGO, firmware_timeout_show,
...
static int do_firmware_uevent(struct firmware_priv *fw_priv,
...
static int fw_realloc_buffer(struct firmware_priv *fw_priv, int min_size) {
  ...
  if (buf->page_array_size < pages_needed) {
    int new_array_size = max(pages_needed, buf->page_array_size * 2);
    struct page **new_pages;

    new_pages = kmalloc(new_array_size * sizeof(void *), GFP_KERNEL);
    if (!new_pages) {
      fw_load_abort(fw_priv);
      return -ENOMEM;
    }
    memcpy(new_pages, buf->pages, buf->page_array_size * sizeof(void *));
    memset(&new_pages[buf->page_array_size], 0,
           sizeof(void *) * (new_array_size - buf->page_array_size));
    kfree(buf->pages);
    buf->pages = new_pages;
  ...
  while (buf->nr_pages < pages_needed) {
    buf->pages[buf->nr_pages] = alloc_page(GFP_KERNEL | __GFP_HIGHMEM);
...
static void firmware_class_timeout_work(struct work_struct *work) {
  struct firmware_priv *fw_priv =
      container_of(work, struct firmware_priv, timeout_work.work);
...
static struct firmware_priv *fw_create_instance(struct firmware *firmware,
                                                struct fw_desc *desc) {
  struct firmware_priv *fw_priv;
  struct device *f_dev;
  ...
  INIT_DELAYED_WORK(&fw_priv->timeout_work, firmware_class_timeout_work);
...
static int _request_firmware_load(struct firmware_priv *fw_priv, bool uevent,
  ...
  struct firmware_buf *buf = fw_priv->buf;
  struct bin_attribute *fw_attr_data =
      buf->dest_addr ? &firmware_direct_attr_data : &firmware_attr_data;
  ...
  if (uevent) {
    ...
    if (timeout != MAX_SCHEDULE_TIMEOUT)
      queue_delayed_work(system_power_efficient_wq, &fw_priv->timeout_work,
                         timeout);
...
static int _request_firmware(struct fw_desc *desc) {
  ...
  if (!fw_get_filesystem_firmware(desc->device, fw->priv, desc->dest_addr,
                                  desc->dest_size))
...
int request_firmware_direct(
    const char *name, struct device *device, phys_addr_t dest_addr,
    size_t dest_size,
    void *(*map_fw_mem)(phys_addr_t phys, size_t size, void *data),
    void (*unmap_fw_mem)(void *virt, size_t size, void *data), void *map_data) {
  struct fw_desc desc;
...
int _request_firmware_nowait(
    struct module *module, bool uevent, const char *name, struct device *device,
    gfp_t gfp, void *context,
    void (*cont)(const struct firmware *fw, void *context), bool nocache,
...
int request_firmware_nowait_direct(
    ...
    void (*unmap_fw_mem)(void *virt, size_t size, void *data), void *map_data) {
  return _request_firmware_nowait(module, uevent, name, device, gfp, context,
                                  cont, true, dest_addr, dest_size, map_fw_mem,
                                  unmap_fw_mem, map_data);
}
...
static void free_fw_cache_entry(struct fw_cache_entry *fce) { kfree(fce); }

static void __async_dev_cache_fw_image(void *fw_entry, async_cookie_t cookie) {
...
static void dev_create_fw_entry(struct device *dev, void *res, void *data) {
  struct fw_name_devm *fwn = res;
...
static int devm_name_match(struct device *dev, void *res, void *match_data) {
  struct fw_name_devm *fwn = res;
...
static void dev_cache_fw_image(struct device *dev, void *data) {
  ...
  devres_for_each_res(dev, fw_name_devm_release, devm_name_match, &fw_cache,
  ...
  list_for_each_entry_safe(fce, fce_next, &todo, list) {
    ...
    if (fce)
      async_schedule_domain(__async_dev_cache_fw_image, (void *)fce,
                            &fw_cache_domain);
  }
}
...
static void __device_uncache_fw_images(void) {
  ...
  while (!list_empty(&fwc->fw_names)) {
    fce = list_entry(fwc->fw_names.next, struct fw_cache_entry, list);
    list_del(&fce->list);
    spin_unlock(&fwc->name_lock);
...
static int fw_pm_notify(struct notifier_block *notify_block, unsigned long mode,
                        void *unused) {
  switch (mode) {
  case PM_HIBERNATION_PREPARE:
  case PM_SUSPEND_PREPARE:
    /*
     * Skip device_cache_fw_images(). On cityman that call
...
static void __init fw_cache_init(void) {
  ...
  INIT_DELAYED_WORK(&fw_cache.work, device_uncache_fw_images_work);
