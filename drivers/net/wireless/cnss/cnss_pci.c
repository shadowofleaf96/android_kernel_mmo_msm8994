...
#define PM_OPTIONS_RESUME_LINK_DOWN (MSM_PCIE_CONFIG_NO_CFG_RESTORE)
...
static int cnss_wlan_vreg_set(struct cnss_wlan_vreg_info *vreg_info,
                              bool state) {
  int ret = 0;
...
static int cnss_wlan_bootstrap_gpio_init(void) {
  ...
  if (ret) {
    pr_err("%s: Can't get GPIO %s, ret = %d\n", __func__,
           WLAN_BOOTSTRAP_GPIO_NAME, ret);
    goto out;
  }

  ret = gpio_direction_output(penv->wlan_bootstrap_gpio, WLAN_BOOTSTRAP_HIGH);
  if (ret) {
...
static int cnss_pinctrl_init(struct cnss_wlan_gpio_info *gpio_info,
  ...
  gpio_info->gpio_state_default =
      pinctrl_lookup_state(gpio_info->pinctrl, CNSS_PINCTRL_STATE_ACTIVE);
  ...
  ret = pinctrl_select_state(gpio_info->pinctrl, gpio_info->gpio_state_default);
...
static int cnss_enable_xtal_ldo(struct platform_device *pdev) {
  ...
  if (!of_get_property(pdev->dev.of_node, WLAN_VREG_XTAL_AON_NAME "-supply",
                       NULL))
    goto enable_xtal;
...
enable_xtal:
  ...
  if (!of_get_property(pdev->dev.of_node, WLAN_VREG_XTAL_NAME "-supply", NULL))
...
static int cnss_wlan_get_resources(struct platform_device *pdev) {
  ...
  if (of_get_property(pdev->dev.of_node, WLAN_VREG_CORE_NAME "-supply", NULL)) {
    vreg_info->wlan_reg_core = regulator_get(&pdev->dev, WLAN_VREG_CORE_NAME);
    ...
    ret = regulator_set_voltage(vreg_info->wlan_reg_core, WLAN_VREG_CORE_MIN,
                                WLAN_VREG_CORE_MAX);
    if (ret) {
  ...
  if (of_get_property(pdev->dev.of_node, WLAN_VREG_IO_NAME "-supply", NULL)) {
    vreg_info->wlan_reg_io = regulator_get(&pdev->dev, WLAN_VREG_IO_NAME);
    if (!IS_ERR(vreg_info->wlan_reg_io)) {
      ret = regulator_set_voltage(vreg_info->wlan_reg_io, WLAN_VREG_IO_MIN,
                                  WLAN_VREG_IO_MAX);
      if (ret) {
  ...
  if (of_get_property(pdev->dev.of_node, WLAN_VREG_SP2T_NAME "-supply", NULL)) {
    vreg_info->wlan_reg_sp2t = regulator_get(&pdev->dev, WLAN_VREG_SP2T_NAME);
    if (!IS_ERR(vreg_info->wlan_reg_sp2t)) {
      ret = regulator_set_voltage(vreg_info->wlan_reg_sp2t, WLAN_VREG_SP2T_MIN,
                                  WLAN_VREG_SP2T_MAX);
      if (ret) {
  ...
  if (of_get_property(pdev->dev.of_node, WLAN_ANT_SWITCH_NAME "-supply",
                      NULL)) {
    vreg_info->ant_switch = regulator_get(&pdev->dev, WLAN_ANT_SWITCH_NAME);
    if (!IS_ERR(vreg_info->ant_switch)) {
      ret =
          regulator_set_voltage(vreg_info->ant_switch, WLAN_ANT_SWITCH_VOLT_MIN,
  ...
  if (of_find_property((&pdev->dev)->of_node, "qcom,wlan-uart-access", NULL))
    penv->cap.cap_flag |= CNSS_HAS_UART_ACCESS;

  if (of_get_property(pdev->dev.of_node, WLAN_SWREG_NAME "-supply", NULL)) {

    vreg_info->soc_swreg = regulator_get(&pdev->dev, WLAN_SWREG_NAME);
    if (IS_ERR(vreg_info->soc_swreg)) {
      pr_err("%s: soc-swreg node not found\n", __func__);
      goto err_reg_get2;
    }
    ret = regulator_set_voltage(vreg_info->soc_swreg, SOC_SWREG_VOLT_MIN,
                                SOC_SWREG_VOLT_MAX);
    if (ret) {
  ...
  gpio_info->prop = true;
  ret = of_get_named_gpio((&pdev->dev)->of_node, gpio_info->name, 0);
  ...
  if (ret >= 0) {
    ...
    if (ret == -EPROBE_DEFER)
      ...
      pr_err("can't get gpio %s ret %d", gpio_info->name, ret);
    goto err_get_gpio;
  }
  ...
  if (of_find_property((&pdev->dev)->of_node, WLAN_BOOTSTRAP_GPIO_NAME, NULL)) {
    penv->wlan_bootstrap_gpio =
        of_get_named_gpio((&pdev->dev)->of_node, WLAN_BOOTSTRAP_GPIO_NAME, 0);
    if (penv->wlan_bootstrap_gpio > 0) {
      ...
      if (ret == -EPROBE_DEFER) {
        pr_debug("%s: Get GPIO %s probe defer\n", __func__,
                 WLAN_BOOTSTRAP_GPIO_NAME);
      } else {
        pr_err("%s: Can't get GPIO %s, ret = %d", __func__,
               WLAN_BOOTSTRAP_GPIO_NAME, ret);
      }
      goto err_gpio_init;
    }
...
static int cnss_wlan_pcie_discover(u32 rc_num) {
  struct pci_bus *bus;
  unsigned int domain = rc_num;
  int ret;
  ...
  /*
   * RC may already be enumerated at boot without the WiFi endpoint
   * (bootstrap was not asserted). Rescan after power-on finds QCA6174.
   */
  bus = pci_find_bus(domain, 0);
  if (!bus) {
    pr_err("%s: PCI bus %u:00 not found\n", __func__, domain);
    return -ENODEV;
  }

  pr_info("%s: bringing up PCIe link on RC %u\n", __func__, domain);
  cnss_msm_pcie_pm_control(MSM_PCIE_RESUME, domain, NULL, 0);

  pr_info("%s: rescanning PCI bus %u:00\n", __func__, domain);
  pci_rescan_bus(bus);
  penv->pcie_link_state = PCIE_LINK_UP;

  return penv->pdev ? 0 : -ENODEV;
}
...
void cnss_setup_fw_files(u16 revision) {
  ...
  case QCA6174_FW_1_1:
    strlcpy(penv->fw_files.image_file, "qwlan11.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.board_data, "bdwlan11.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.otp_data, "otp11.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.utf_file, "utf11.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.utf_board_data, "utfbd11.bin", CNSS_MAX_FILE_NAME);
    break;

  case QCA6174_FW_1_3:
    strlcpy(penv->fw_files.image_file, "qwlan13.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.board_data, "bdwlan13.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.otp_data, "otp13.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.utf_file, "utf13.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.utf_board_data, "utfbd13.bin", CNSS_MAX_FILE_NAME);
    break;

  case QCA6174_FW_2_0:
    strlcpy(penv->fw_files.image_file, "qwlan20.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.board_data, "bdwlan20.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.otp_data, "otp20.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.utf_file, "utf20.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.utf_board_data, "utfbd20.bin", CNSS_MAX_FILE_NAME);
    break;

  case QCA6174_FW_3_0:
  case QCA6174_FW_3_2:
    strlcpy(penv->fw_files.image_file, "qwlan30.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.board_data, "bdwlan30.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.otp_data, "otp30.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.utf_file, "utf30.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.utf_board_data, "utfbd30.bin", CNSS_MAX_FILE_NAME);
    break;

  default:
    strlcpy(penv->fw_files.image_file, "qwlan.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.board_data, "bdwlan.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.otp_data, "otp.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.utf_file, "utf.bin", CNSS_MAX_FILE_NAME);
    strlcpy(penv->fw_files.utf_board_data, "utfbd.bin", CNSS_MAX_FILE_NAME);
    break;
  }
}
...
int cnss_get_fw_files_for_target(struct cnss_fw_files *pfw_files,
  ...
  default:
    ...
    pr_err("%s version mismatch 0x%X 0x%X", __func__, target_type,
           target_version);
    break;
  }
  return 0;
...
static int get_image_file(const u8 *index_info, u8 *file_name, u32 *type,
                          u32 *segment_idx) {
  ...
  memcpy(file_name, index_info + TYPE_LENGTH + TYPE_LENGTH, FW_FILENAME_LENGTH);
...
static void print_allocated_image_table(void) {
  ...
  while (seg++ < penv->fw_seg_count) {
    dump_addr = (u8 *)pseg_mem->cpu_region + sizeof(struct region_desc);
    for (count = 0; count < pseg_mem->size - sizeof(struct region_desc);
         count++)
  ...
  while (seg++ < penv->bdata_seg_count) {
    dump_addr = (u8 *)p_bdata_seg_mem->cpu_region + sizeof(struct region_desc);
    for (count = 0; count < p_bdata_seg_mem->size - sizeof(struct region_desc);
         count++)
      pr_debug("%02x ", dump_addr[count]);
...
static void free_allocated_image_table(void) {
  ...
  while (seg++ < penv->fw_seg_count) {
    dma_free_coherent(dev, pseg_mem->size, pseg_mem->cpu_region,
                      pseg_mem->dma_region);
    pseg_mem++;
  }
  if (penv->fw_cpu)
    dma_free_coherent(dev, sizeof(struct segment_desc) * MAX_NUM_OF_SEGMENTS,
...
static int cnss_setup_fw_image_table(int mode) {
  ...
  vaddr = dma_alloc_coherent(dev, image_desc_size, &paddr, GFP_KERNEL);
  ...
  while (file_size >= PER_FILE_DATA && image_pos < image_desc_size &&
    ...
    ret =
        get_image_file(index_info + index_pos, image_file, &type, &segment_idx);
    if (ret == -EINVAL)
      goto err_free;

    file_size -= ret;
    index_pos += ret;
    pseg = vaddr + image_pos + sizeof(struct image_desc_hdr);
    ...
    case SEG_NON_SECURE_DATA:
      ...
      if (ret || !fw_image || !fw_image->data || !fw_image->size) {
        pr_err("cnss: image file read failed %s", image_file);
        goto err_free;
      }
      ...
      reg_desc =
          dma_alloc_coherent(dev, sizeof(struct region_desc) + fw_image->size,
                             &dma_addr, GFP_KERNEL);
      if (!reg_desc) {
      ...
      pseg->addr_count = 1;
      memcpy((u8 *)reg_desc + sizeof(struct region_desc), fw_image->data,
             fw_image->size);
      ...
      pseg_mem[*pseg_count].size = sizeof(struct region_desc) + fw_image->size;
...
int cnss_get_fw_image(struct image_desc_info *image_desc_info) {
  if (!image_desc_info || !penv || !penv->fw_seg_count ||
      !penv->bdata_seg_count)
    return -EINVAL;
...
static DEVICE_ATTR(wlan_setup, S_IRUSR, wlan_setup_show, NULL);
...
static int cnss_smmu_init(struct device *dev) {
  ...
  mapping = arm_iommu_create_mapping(&platform_bus_type, penv->smmu_iova_start,
                                     penv->smmu_iova_len, 16);
  ...
  ret = iommu_domain_set_attr(mapping->domain, DOMAIN_ATTR_COHERENT_HTW_DISABLE,
                              &disable_htw);
  if (ret) {
...
int cnss_msm_pcie_enumerate(u32 rc_idx) { return msm_pcie_enumerate(rc_idx); }
...
int cnss_msm_pcie_recover_config(struct pci_dev *dev) { return -ENODEV; }

int cnss_msm_pcie_register_event(struct msm_pcie_register_event *reg) {
  return -ENODEV;
}

int cnss_msm_pcie_enumerate(u32 rc_idx) { return -EPROBE_DEFER; }
#endif
...
static int cnss_wlan_pci_probe(struct pci_dev *pdev,
  ...
  case QCA6180_DEVICE_ID:
    pci_read_config_word(pdev, QCA6180_REV_ID_OFFSET, &penv->revision_id);
    break;
  ...
  if (penv->pcie_link_state) {
    ...
    ret = cnss_msm_pcie_pm_control(
        MSM_PCIE_SUSPEND, cnss_get_pci_dev_bus_number(pdev), pdev, PM_OPTIONS);
    if (ret) {
  ...
  cpu_addr =
      dma_alloc_coherent(dev, EVICT_BIN_MAX_SIZE, &dma_handle, GFP_KERNEL);
  ...
  cnss_seg_info = devm_kzalloc(dev, sizeof(*cnss_seg_info), GFP_KERNEL);
  if (!cnss_seg_info) {
...
static int cnss_pm_notify(struct notifier_block *b, unsigned long event,
                          void *p) {
  switch (event) {
...
static ssize_t fw_image_setup_store(struct device *dev,
  ...
  if (val == FW_IMAGE_FTM || val == FW_IMAGE_MISSION || val == FW_IMAGE_BDATA) {
    pr_info("%s: fw image setup triggered %d\n", __func__, val);
    ret = cnss_setup_fw_image_table(val);
    if (ret != 0) {
      pr_err("%s: Invalid parsing of FW image files %d", __func__, ret);
      return -EINVAL;
    }
...
static DEVICE_ATTR(fw_image_setup, S_IRUSR | S_IWUSR, fw_image_setup_show,
                   fw_image_setup_store);
...
void cnss_pci_events_cb(struct msm_pcie_notify *notify) {
  ...
  case MSM_PCIE_EVENT_WAKEUP:
    if (penv->monitor_wake_intr && atomic_read(&penv->auto_suspended)) {
      penv->monitor_wake_intr = false;
...
static void cnss_wlan_memory_expansion(void) {
  ...
  if (!filename || filename[0] == '\0') {
    pr_debug("cnss: evicted_data filename is empty, skipping\n");
    goto end;
  }
...
int cnss_pcie_set_wlan_mac_address(const u8 *in, uint32_t len) {
  ...
  for (; iter < no_of_mac_addr; ++iter, temp += ETH_ALEN, in += ETH_ALEN) {
...
again:
  ...
  if (!pdev) {
    u32 rc_num = 0;
    struct device *cnss_dev = &penv->pldev->dev;

    pr_debug("%s: invalid pdev. register pci device\n", __func__);
    ret = pci_register_driver(&cnss_wlan_pci_driver);
    if (ret) {
      /* Expected when cnss_probe already registered the driver. */
      pr_info("%s: pci driver already registered (%d)\n", __func__, ret);
      ret = 0;
    }

    pdev = penv->pdev;
    if (!pdev) {
      if (of_property_read_u32(cnss_dev->of_node, "qcom,wlan-rc-num",
                               &rc_num)) {
      ...
      ret = cnss_wlan_pcie_discover(rc_num);
      if (ret) {
        pr_err("%s: Failed to discover WLAN PCIe device on RC%u: %d\n",
               __func__, rc_num, ret);
        goto err_pcie_reg;
      }
      pdev = penv->pdev;
  ...
  penv->event_reg.events = MSM_PCIE_EVENT_LINKDOWN | MSM_PCIE_EVENT_WAKEUP;
  ...
  if (!penv->pcie_link_state && !penv->pcie_link_down_ind) {
    ret = cnss_msm_pcie_pm_control(
        MSM_PCIE_RESUME, cnss_get_pci_dev_bus_number(pdev), pdev, PM_OPTIONS);
    if (ret) {
    ...
    ret = cnss_msm_pcie_pm_control(MSM_PCIE_RESUME,
                                   cnss_get_pci_dev_bus_number(pdev), pdev,
                                   PM_OPTIONS_RESUME_LINK_DOWN);
...
bus_request:
  if (penv->bus_client)
    msm_bus_scale_client_update_request(penv->bus_client, CNSS_BUS_WIDTH_NONE);
}
...
int cnss_wlan_pm_control(bool vote) {
  ...
  return cnss_msm_pcie_pm_control(
      ...
      cnss_get_pci_dev_bus_number(penv->pdev), penv->pdev, PM_OPTIONS);
}
EXPORT_SYMBOL(cnss_wlan_pm_control);
#endif

void cnss_lock_pm_sem(void) { down_read(&cnss_pm_sem); }
EXPORT_SYMBOL(cnss_lock_pm_sem);

void cnss_release_pm_sem(void) { up_read(&cnss_pm_sem); }
...
err_wlan_reinit:
  pci_save_state(pdev);
  penv->saved_state = cnss_pci_store_saved_state(pdev);
  cnss_msm_pcie_pm_control(MSM_PCIE_SUSPEND, cnss_get_pci_dev_bus_number(pdev),
                           pdev, PM_OPTIONS);
  penv->pcie_link_state = PCIE_LINK_DOWN;
...
static void cnss_crash_shutdown(const struct subsys_desc *subsys) {
  ...
  strlcpy(penv->dump_data.name, CNSS_DUMP_NAME, sizeof(penv->dump_data.name));
...
static int cnss_probe(struct platform_device *pdev) {
  ...
  if (of_property_read_u32(dev->of_node, "qcom,wlan-ramdump-dynamic",
                           &ramdump_size) == 0) {
    penv->ramdump_addr = dma_alloc_coherent(&pdev->dev, ramdump_size,
                                            &penv->ramdump_phys, GFP_KERNEL);

    if (penv->ramdump_addr)
      penv->ramdump_size = ramdump_size;
    penv->ramdump_dynamic = true;
  } else {
    res = platform_get_resource_byname(penv->pldev, IORESOURCE_MEM, "ramdump");
    if (res) {
      penv->ramdump_phys = res->start;
      ramdump_size = resource_size(res);
      penv->ramdump_addr = ioremap(penv->ramdump_phys, ramdump_size);
  ...
  penv->ramdump_dev =
      create_ramdump_device(penv->subsysdesc.name, penv->subsysdesc.dev);
  if (!penv->ramdump_dev) {
    ret = -ENOMEM;
    goto err_ramdump_create;
  }
...
skip_ramdump:
  ...
  if (penv->notify_modem_status) {
    penv->modem_notify_handler = subsys_notif_register_notifier(
        penv->esoc_desc ? penv->esoc_desc->name : "modem", &mnb);
  ...
  if (of_property_read_u32_array(dev->of_node, "qcom,wlan-smmu-iova-address",
  ...
  /*
   * Defer PCI driver registration until the WLAN driver loads. At boot
   * the bootstrap GPIO is not asserted yet, so early registration leaves
   * pdev NULL and breaks delayed qcacld init via fwpath.
   */
  ...
  if (penv->bus_scale_table) {
    penv->bus_client = msm_bus_scale_register_client(penv->bus_scale_table);
...
err_ramdump_create:
  if (penv->ramdump_addr) {
    if (penv->ramdump_dynamic) {
      dma_free_coherent(&pdev->dev, penv->ramdump_size, penv->ramdump_addr,
                        penv->ramdump_phys);
    } else {
...
static const struct of_device_id cnss_dt_match[] = {{.compatible = "qcom,cnss"},
                                                    {}};
...
int cnss_pci_request_bus_bandwidth(int bandwidth) {
  ...
  case CNSS_BUS_WIDTH_HIGH:
    ret = msm_bus_scale_client_update_request(penv->bus_client, bandwidth);
    if (!ret) {
...
static int __cnss_event_reg(struct device *dev) {
  ...
  event_reg->events = MSM_PCIE_EVENT_LINKDOWN | MSM_PCIE_EVENT_WAKEUP;
...
static int __cnss_restore_pci_config_space(struct device *dev) {
  ...
  if (penv->saved_state)
    ret = cnss_pci_load_and_free_saved_state(pdev, &penv->saved_state);
  pci_restore_state(pdev);
