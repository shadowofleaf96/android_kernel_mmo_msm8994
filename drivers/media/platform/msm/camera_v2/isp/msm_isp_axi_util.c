...
#define SRC_TO_INTF(src)                                                       \
  ((src < RDI_INTF_0 || src == VFE_AXI_SRC_MAX)                                \
       ? VFE_PIX_0                                                             \
...
int msm_isp_axi_create_stream(
  ...
  if (axi_data->stream_info[i].state != AVALIABLE) {
    pr_err("%s:%d invalid state %d expected %d for src %d\n", __func__,
           __LINE__, axi_data->stream_info[i].state, AVALIABLE, i);
    return -EINVAL;
  }
  ...
  stream_cfg_cmd->axi_stream_handle = (++axi_data->stream_handle_cnt) << 8 | i;

  memset(&axi_data->stream_info[i], 0, sizeof(struct msm_vfe_axi_stream));
  ...
  axi_data->stream_info[i].stream_handle = stream_cfg_cmd->axi_stream_handle;
...
void msm_isp_axi_destroy_stream(struct msm_vfe_axi_shared_data *axi_data,
                                int stream_idx) {
...
int msm_isp_validate_axi_request(
  ...
  default:
    msm_isp_print_fourcc_error(__func__, stream_cfg_cmd->output_format);
    return rc;
  }
  ...
  if ((stream_info->num_planes > 1) &&
      (axi_data->hw_info->num_comp_mask - axi_data->num_used_composite_mask <
       1)) {
  ...
  for (i = 0; i < stream_info->num_planes; i++) {
    ...
    stream_info->max_width =
        max(stream_info->max_width, stream_cfg_cmd->plane_cfg[i].output_width);
  }
...
static uint32_t
msm_isp_axi_get_plane_size(struct msm_vfe_axi_stream *stream_info,
                           int plane_idx) {
  uint32_t size = 0;
  ...
  case V4L2_PIX_FMT_GREY:
    size =
        plane_cfg[plane_idx].output_height * plane_cfg[plane_idx].output_width;
    break;
  case V4L2_PIX_FMT_SBGGR10:
  case V4L2_PIX_FMT_SGBRG10:
  case V4L2_PIX_FMT_SGRBG10:
  ...
  /*TD: Add more image format*/
  default:
    msm_isp_print_fourcc_error(__func__, stream_info->output_format);
    break;
  }
  return size;
}
  ...
  for (i = 0; i < stream_info->num_planes; i++) {
    for (j = 0; j < axi_data->hw_info->num_wm; j++) {
      if (!axi_data->free_wm[j]) {
        axi_data->free_wm[j] = stream_info->stream_handle;
        axi_data->wm_image_size[j] = msm_isp_axi_get_plane_size(stream_info, i);
...
void msm_isp_axi_reserve_comp_mask(struct msm_vfe_axi_shared_data *axi_data,
                                   struct msm_vfe_axi_stream *stream_info) {
  int i;
  uint8_t comp_mask = 0;
  for (i = 0; i < stream_info->num_planes; i++)
  ...
  for (i = 0; i < axi_data->hw_info->num_comp_mask; i++) {
    if (!axi_data->composite_info[i].stream_handle) {
      axi_data->composite_info[i].stream_handle = stream_info->stream_handle;
      axi_data->composite_info[i].stream_composite_mask = comp_mask;
...
void msm_isp_axi_free_comp_mask(struct msm_vfe_axi_shared_data *axi_data,
                                struct msm_vfe_axi_stream *stream_info) {
  axi_data->composite_info[stream_info->comp_mask_index].stream_composite_mask =
      0;
  axi_data->composite_info[stream_info->comp_mask_index].stream_handle = 0;
  axi_data->num_used_composite_mask--;
}
...
int msm_isp_axi_check_stream_state(
  ...
  for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
    ...
    stream_info =
        &axi_data->stream_info[HANDLE_TO_IDX(stream_cfg_cmd->stream_handle[i])];
    spin_lock_irqsave(&stream_info->lock, flags);
    if (stream_info->state != valid_state) {
      if ((stream_info->state == PAUSING || stream_info->state == PAUSED ||
           stream_info->state == RESUME_PENDING ||
           stream_info->state == RESUMING || stream_info->state == UPDATING) &&
...
void msm_isp_cfg_framedrop_reg(struct vfe_device *vfe_dev,
  ...
  if (stream_info->stream_type == BURST_STREAM) {
    if (stream_info->runtime_burst_frame_count == 0) {
      ...
      framedrop_pattern = (1 << stream_info->runtime_num_burst_capture) - 1;
      framedrop_pattern <<= stream_info->runtime_init_frame_drop;
      ...
      framedrop_period = 30 + (stream_info->framedrop_altern_cnt++ & 1);
    }
  ...
  if ((stream_info->stream_src < RDI_INTF_0) && vfe_dev->is_split &&
    ...
    if (!axi_data || (vfe0_stream_handle >= VFE_AXI_SRC_MAX)) {
      ...
      vfe0_stream_info = &axi_data->stream_info[vfe0_stream_handle];
      vfe_dev->hw_info->vfe_ops.axi_ops.cfg_framedrop(
          vfe_dev->dual_vfe_res->vfe_base[ISP_VFE0], vfe0_stream_info,
          framedrop_pattern, framedrop_period);
      vfe_dev->hw_info->vfe_ops.axi_ops.cfg_framedrop(
          vfe_dev->vfe_base, stream_info, framedrop_pattern, framedrop_period);
    }
  } else if (!vfe_dev->is_split || ((stream_info->stream_src >= RDI_INTF_0) &&
...
void msm_isp_update_framedrop_reg(struct vfe_device *vfe_dev,
  ...
  for (i = 0; i < MAX_NUM_STREAM; i++) {
    if (SRC_TO_INTF(axi_data->stream_info[i].stream_src) != frame_src) {
      continue;
    }
    ...
    if (stream_info->runtime_framedrop_update_burst) {
      ...
      if ((stream_info->runtime_burst_frame_count < BURST_SKIP_THRESHOLD) &&
...
void msm_isp_check_for_output_error(struct vfe_device *vfe_dev,
  ...
  if (!vfe_dev->reg_updated) {
    output_info->regs_not_updated = vfe_dev->reg_update_requested;
  }
  for (i = 0; i < MAX_NUM_STREAM; i++) {
    stream_info = &axi_data->stream_info[i];
    stream_idx = HANDLE_TO_IDX(stream_info->stream_handle);

    if ((stream_info->state != ACTIVE && stream_info->state != RESUMING) ||
        (SRC_TO_INTF(stream_info->stream_src) != VFE_PIX_0))
      continue;

    if (stream_info->controllable_output &&
        stream_info->undelivered_request_cnt && !vfe_dev->reg_updated) {
      if (msm_isp_drop_frame(vfe_dev, stream_info, ts, output_info)) {
        pr_debug("drop frame failed\n");
      }
    }
    if (stream_info->state == RESUMING && !stream_info->controllable_output) {
      pr_err("%s: axi_updating_mask stream_id %x frame_id %d\n", __func__,
             stream_idx,
             vfe_dev->axi_data.src_info[SRC_TO_INTF(stream_info->stream_src)]
                 .frame_id);
      output_info->axi_updating_mask |= 1 << stream_idx;
    }
  }
  ...
  if (vfe_dev->error_info.framedrop_flag) {
    for (i = 0; i < MAX_NUM_STREAM; i++) {
      ...
      if (vfe_dev->error_info.stream_framedrop_count[i]) {
        ...
        vfe_dev->error_info.stream_framedrop_count[i] = 0;
      }
    }
...
void msm_isp_notify(struct vfe_device *vfe_dev, uint32_t event_type,
  ...
  case ISP_EVENT_SOF: {
    ...
    if (frame_src == VFE_PIX_0) {
      ...
      msm_isp_check_for_output_error(vfe_dev, ts, &event_data.u.output_info);

      vfe_dev->axi_data.src_info[frame_src].frame_id +=
          vfe_dev->axi_data.src_info[frame_src].sof_counter_step;
      if (!src_info->frame_id && !src_info->reg_update_frame_id &&
          ((src_info->frame_id - src_info->reg_update_frame_id) >
           (MAX_REG_UPDATE_THRESHOLD * src_info->sof_counter_step))) {
        pr_err("%s:%d reg_upfate not received for %d frames\n", __func__,
               __LINE__, src_info->frame_id - src_info->reg_update_frame_id);
        ...
        msm_isp_send_event(vfe_dev, ISP_EVENT_REG_UPDATE_MISSING, &error_event);
      }
...
void msm_isp_calculate_framedrop(
  ...
  framedrop_period =
      msm_isp_get_framedrop_period(stream_cfg_cmd->frame_skip_pattern);
  stream_info->frame_skip_pattern = stream_cfg_cmd->frame_skip_pattern;
  ...
  if (stream_cfg_cmd->init_frame_drop < framedrop_period) {
    stream_info->framedrop_pattern <<= stream_cfg_cmd->init_frame_drop;
  ...
  if (stream_cfg_cmd->burst_count > 0) {
    ...
    stream_info->num_burst_capture = stream_cfg_cmd->burst_count;
    stream_info->burst_frame_count =
        stream_cfg_cmd->init_frame_drop +
        (stream_cfg_cmd->burst_count - 1) * framedrop_period + 1;
  } else {
...
void msm_isp_calculate_bandwidth(struct msm_vfe_axi_shared_data *axi_data,
                                 struct msm_vfe_axi_stream *stream_info) {
  int bpp = 0;
  if (stream_info->stream_src < RDI_INTF_0) {
    stream_info->bandwidth = (axi_data->src_info[VFE_PIX_0].pixel_clock /
    ...
    if (rdi < VFE_SRC_MAX)
      stream_info->bandwidth = (axi_data->src_info[rdi].pixel_clock / 8) * bpp;
    else
...
static inline void
msm_isp_get_avtimer_ts(struct msm_isp_timestamp *time_stamp) {
  int rc = 0;
  ...
  if (rc < 0) {
    ...
    pr_debug("%s: AVTimer TS = %u:%u\n", __func__, (uint32_t)(avtimer_tick),
             avtimer_usec);
  }
}
#else
void msm_isp_start_avtimer(void) { pr_err("AV Timer is not supported\n"); }
...
int msm_isp_request_axi_stream(struct vfe_device *vfe_dev, void *arg) {
  ...
  rc = msm_isp_axi_create_stream(&vfe_dev->axi_data, stream_cfg_cmd);
  if (rc) {
    pr_err("%s: create stream failed\n", __func__);
    return rc;
  }

  rc = msm_isp_validate_axi_request(&vfe_dev->axi_data, stream_cfg_cmd);
  if (rc) {
    pr_err("%s: Request validation failed\n", __func__);
    if (HANDLE_TO_IDX(stream_cfg_cmd->axi_stream_handle) < MAX_NUM_STREAM)
      msm_isp_axi_destroy_stream(
          &vfe_dev->axi_data, HANDLE_TO_IDX(stream_cfg_cmd->axi_stream_handle));
    return rc;
  }
  stream_info =
      &vfe_dev->axi_data
  ...
  vfe_dev->reg_update_requested &= ~(BIT(SRC_TO_INTF(stream_info->stream_src)));
  ...
  if (stream_info->num_planes > 1) {
    msm_isp_axi_reserve_comp_mask(&vfe_dev->axi_data, stream_info);
    vfe_dev->hw_info->vfe_ops.axi_ops.cfg_comp_mask(vfe_dev, stream_info);
  } else {
    vfe_dev->hw_info->vfe_ops.axi_ops.cfg_wm_irq_mask(vfe_dev, stream_info);
  }

  for (i = 0; i < stream_info->num_planes; i++) {
    vfe_dev->hw_info->vfe_ops.axi_ops.cfg_wm_reg(vfe_dev, stream_info, i);
...
int msm_isp_release_axi_stream(struct vfe_device *vfe_dev, void *arg) {
  ...
  stream_info =
      &axi_data->stream_info[HANDLE_TO_IDX(stream_release_cmd->stream_handle)];
  ...
  for (i = 0; i < stream_info->num_planes; i++) {
    vfe_dev->hw_info->vfe_ops.axi_ops.clear_wm_reg(vfe_dev, stream_info, i);

    vfe_dev->hw_info->vfe_ops.axi_ops.clear_wm_xbar_reg(vfe_dev, stream_info,
                                                        i);
  }

  if (stream_info->num_planes > 1) {
    vfe_dev->hw_info->vfe_ops.axi_ops.clear_comp_mask(vfe_dev, stream_info);
    msm_isp_axi_free_comp_mask(&vfe_dev->axi_data, stream_info);
  } else {
    vfe_dev->hw_info->vfe_ops.axi_ops.clear_wm_irq_mask(vfe_dev, stream_info);
  }
...
static void
msm_isp_axi_stream_enable_cfg(struct vfe_device *vfe_dev,
                              struct msm_vfe_axi_stream *stream_info) {
  int i;
  struct msm_vfe_axi_shared_data *axi_data = &vfe_dev->axi_data;
  ...
  for (i = 0; i < stream_info->num_planes; i++) {
    if (stream_info->state == START_PENDING ||
        stream_info->state == RESUME_PENDING) {
      vfe_dev->hw_info->vfe_ops.axi_ops.enable_wm(vfe_dev, stream_info->wm[i],
                                                  1);
    } else {
      vfe_dev->hw_info->vfe_ops.axi_ops.enable_wm(vfe_dev, stream_info->wm[i],
                                                  0);
...
void msm_isp_axi_stream_update(struct vfe_device *vfe_dev,
  ...
  for (i = 0; i < MAX_NUM_STREAM; i++) {
    if (SRC_TO_INTF(axi_data->stream_info[i].stream_src) != frame_src) {
      ISP_DBG("%s stream_src %d frame_src %d\n", __func__,
              SRC_TO_INTF(axi_data->stream_info[i].stream_src), frame_src);
      continue;
    }
    if (axi_data->stream_info[i].state == UPDATING)
      ...
      axi_data->stream_info[i].state =
          axi_data->stream_info[i].state == START_PENDING ? STARTING : STOPPING;
    } else if (axi_data->stream_info[i].state == STARTING ||
               axi_data->stream_info[i].state == STOPPING) {
      axi_data->stream_info[i].state =
          axi_data->stream_info[i].state == STARTING ? ACTIVE : INACTIVE;
    }
  }
  ...
  if (vfe_dev->axi_data.pipeline_update == DISABLE_CAMIF ||
      (vfe_dev->axi_data.pipeline_update == DISABLE_CAMIF_IMMEDIATELY)) {
...
static void
msm_isp_reload_ping_pong_offset(struct vfe_device *vfe_dev,
                                struct msm_vfe_axi_stream *stream_info) {
  int i, j;
  uint32_t flag;
...
void msm_isp_axi_cfg_update(struct vfe_device *vfe_dev,
  ...
  for (i = 0; i < MAX_NUM_STREAM; i++) {
    ...
    if (stream_info->state == PAUSING) {
      ...
      for (j = 0; j < stream_info->num_planes; j++)
        vfe_dev->hw_info->vfe_ops.axi_ops.cfg_wm_reg(vfe_dev, stream_info, j);
      /*Resume AXI*/
      stream_info->state = RESUME_PENDING;
      msm_isp_axi_stream_enable_cfg(vfe_dev, &axi_data->stream_info[i]);
  ...
  if (num_stream)
    update_state = atomic_dec_return(&axi_data->axi_cfg_update[frame_src]);
}
...
static int msm_isp_get_done_buf(struct vfe_device *vfe_dev,
  ...
  for (i = 0; i < stream_info->num_planes; i++) {
    if (pingpong_bit != (~(pingpong_status >> stream_info->wm[i]) & 0x1)) {
  ...
  for (j = 0; j < MAX_NUM_STREAM; j++) {
    ...
    for (pingpong_bit = 0; pingpong_bit < 2; pingpong_bit++) {
      for (i = 0; i < stream_info->num_planes; i++) {
        ...
        pr_err("%s: stream_id %x ping-pong %d plane %d start_addr %x "
               "addr_offset %x len %lx stride %d scanline %d\n",
               __func__, stream_info->stream_id, pingpong_bit, i,
               (uint32_t)buf->mapped_info[i].paddr,
               stream_info->plane_cfg[i].plane_addr_offset,
               buf->mapped_info[i].len, stream_info->plane_cfg[i].output_stride,
               stream_info->plane_cfg[i].output_scan_lines);
      }
    }
  }
...
static int msm_isp_cfg_ping_pong_address(struct vfe_device *vfe_dev,
  ...
  if (!stream_info->controllable_output) {
    ...
    queue_req = list_first_entry_or_null(
        &stream_info->request_q, struct msm_vfe_frame_request_queue, list);
    if (!queue_req)
      return 0;

    bufq_handle = stream_info->bufq_handle[queue_req->buff_queue_id];
  ...
  if (valid_address == 1) {
    rc = vfe_dev->buf_mgr->ops->get_buf(vfe_dev->buf_mgr, vfe_dev->pdev->id,
                                        bufq_handle, &buf, &buf_cnt);
  ...
  for (i = 0; i < stream_info->num_planes; i++) {
    ...
    if ((stream_info->stream_src < RDI_INTF_0) && vfe_dev->is_split &&
      ...
      if (!dual_vfe_res->vfe_base[ISP_VFE0] ||
        ...
        pr_err(
            "%s:%d failed vfe0 %pK %pK vfe %pK %pK\n", __func__, __LINE__,
            dual_vfe_res->vfe_base[ISP_VFE0], dual_vfe_res->axi_data[ISP_VFE0],
            dual_vfe_res->vfe_base[ISP_VFE1], dual_vfe_res->axi_data[ISP_VFE1]);
      } else {
        for (vfe_id = 0; vfe_id < MAX_VFE; vfe_id++) {
          vfe_dev->hw_info->vfe_ops.axi_ops.update_ping_pong_addr(
              dual_vfe_res->vfe_base[vfe_id],
              dual_vfe_res->axi_data[vfe_id]->stream_info[stream_idx].wm[i],
              pingpong_status,
              paddr + dual_vfe_res->axi_data[vfe_id]
               ...
               (stream_info->stream_src >= RDI_INTF_0 &&
                stream_info->stream_src <= RDI_INTF_2)) {
      vfe_dev->hw_info->vfe_ops.axi_ops.update_ping_pong_addr(
          vfe_dev->vfe_base, stream_info->wm[i], pingpong_status,
          paddr + stream_info->plane_cfg[i].plane_addr_offset);
    }
  }
  ...
  return 0;
buf_error:
  vfe_dev->buf_mgr->ops->put_buf(vfe_dev->buf_mgr, buf->bufq_handle,
                                 buf->buf_idx);
  return rc;
}
...
static void msm_isp_process_done_buf(struct vfe_device *vfe_dev,
  ...
  if (SRC_TO_INTF(stream_info->stream_src) >= VFE_SRC_MAX) {
    ...
    rc = vfe_dev->buf_mgr->ops->put_buf(vfe_dev->buf_mgr, buf->bufq_handle,
                                        buf->buf_idx);
    return;
  }

  if (stream_info->stream_type != BURST_STREAM &&
      (stream_info->sw_skip.stream_src_mask & (1 << stream_info->stream_src))) {
    ...
    if (stream_info->sw_skip.skip_mode == SKIP_ALL) {
      ...
      spin_lock_irqsave(&stream_info->lock, flags);
      memset(&stream_info->sw_skip, 0, sizeof(struct msm_isp_sw_framskip));
      spin_unlock_irqrestore(&stream_info->lock, flags);
    }
  }
  ...
  rc = vfe_dev->buf_mgr->ops->get_buf_src(vfe_dev->buf_mgr, buf->bufq_handle,
                                          &buf_src);
  if (rc != 0) {
  ...
  if (stream_info->buf_divert && rc == 0 &&
    ...
    rc = vfe_dev->buf_mgr->ops->buf_divert(vfe_dev->buf_mgr, buf->bufq_handle,
                                           buf->buf_idx, time_stamp, frame_id);
  } else {
  ...
  if (rc > 0) {
    ...
    if (buf->frame_id != frame_id) {
      ...
      error_event.frame_id = vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id;
      error_event.u.error_info.err_type = ISP_ERROR_FRAME_ID_MISMATCH;
      rc = vfe_dev->buf_mgr->ops->put_buf(vfe_dev->buf_mgr, buf->bufq_handle,
                                          buf->buf_idx);
    ...
    buf_event.u.buf_done.output_format = stream_info->runtime_output_format;
    if (stream_info->buf_divert && buf_src != MSM_ISP_BUFFER_SRC_SCRATCH) {
      ISP_DBG("%s: vfe_id %d send buf_divert buf-id %d bufq %x\n", __func__,
              vfe_dev->pdev->id, buf->buf_idx, buf->bufq_handle);

      msm_isp_send_event(vfe_dev, ISP_EVENT_BUF_DIVERT, &buf_event);
    } else {
      ...
      vfe_dev->buf_mgr->ops->buf_done(vfe_dev->buf_mgr, buf->bufq_handle,
                                      buf->buf_idx, time_stamp, frame_id,
...
int msm_isp_drop_frame(struct vfe_device *vfe_dev,
  ...
  if (!vfe_dev || !stream_info || !ts || !output_info) {
    pr_err("%s %d  vfe_dev %pK stream_info %pK ts %pK op_info %pK\n", __func__,
           __LINE__, vfe_dev, stream_info, ts, output_info);
    return -EINVAL;
  }
  ...
  spin_lock_irqsave(&stream_info->lock, flags);
  rc = msm_isp_get_done_buf(vfe_dev, stream_info, pingpong_status, &done_buf);
  if (rc < 0) {
  ...
  if (stream_info->stream_type == CONTINUOUS_STREAM ||
      stream_info->runtime_num_burst_capture > 1)
    msm_isp_cfg_ping_pong_address(vfe_dev, stream_info, pingpong_status, 1, 1);
  ...
  if (done_buf) {
    vfe_dev->buf_mgr->ops->buf_done(vfe_dev->buf_mgr, done_buf->bufq_handle,
                                    done_buf->buf_idx, &ts->buf_time, frame_id,
                                    stream_info->runtime_output_format);
    ...
    pr_err("%s: output_err_mask stream_id %x frame_id %d\n", __func__,
           bufq->stream_id, frame_id);
    output_info->output_err_mask |= 1 << (done_buf->bufq_handle & 0xFF);
  }
  return 0;
}

static enum msm_isp_camif_update_state msm_isp_get_camif_update_state(
    struct vfe_device *vfe_dev,
    struct msm_vfe_axi_stream_cfg_cmd *stream_cfg_cmd) {
  int i;
  struct msm_vfe_axi_stream *stream_info;
  ...
  if ((pix_stream_cnt) &&
    ...
    if (cur_pix_stream_cnt == 0 && pix_stream_cnt &&
      ...
      return ENABLE_CAMIF;
    else if (cur_pix_stream_cnt && (cur_pix_stream_cnt - pix_stream_cnt) == 0 &&
             stream_cfg_cmd->cmd == STOP_STREAM)
...
static int msm_isp_update_stream_bandwidth(struct vfe_device *vfe_dev) {
  ...
  for (i = 0; i < MAX_NUM_STREAM; i++) {
    stream_info = &axi_data->stream_info[i];
    if (stream_info->state == ACTIVE || stream_info->state == START_PENDING) {
      if (stream_info->stream_src < RDI_INTF_0) {
  ...
  if (total_streams == 1)
    rc = msm_isp_update_bandwidth(
        ISP_VFE0 + vfe_dev->pdev->id, total_bandwidth,
...
static int
msm_isp_axi_wait_for_cfg_done(struct vfe_device *vfe_dev,
...
static int
msm_isp_init_stream_ping_pong_reg(struct vfe_device *vfe_dev,
                                  struct msm_vfe_axi_stream *stream_info) {
  int rc = 0;
  uint8_t valid_address;

  /*Set address for both PING & PONG register */
  rc = msm_isp_cfg_ping_pong_address(vfe_dev, stream_info, VFE_PING_FLAG, 1, 0);
  if (rc < 0) {
  ...
  rc = msm_isp_cfg_ping_pong_address(vfe_dev, stream_info, VFE_PONG_FLAG,
                                     valid_address, 0);
  if (rc < 0) {
...
static void
msm_isp_deinit_stream_ping_pong_reg(struct vfe_device *vfe_dev,
                                    struct msm_vfe_axi_stream *stream_info) {
  int i;
  for (i = 0; i < 2; i++) {
    struct msm_isp_buffer *buf;
...
static void msm_isp_get_stream_wm_mask(struct msm_vfe_axi_stream *stream_info,
...
int msm_isp_axi_halt(struct vfe_device *vfe_dev,
  ...
  if (halt_cmd->overflow_detected) {
    ...
    if (vfe_dev->error_info.overflow_recover_irq_mask0 == 0) {
      vfe_dev->hw_info->vfe_ops.core_ops.get_irq_mask(
          vfe_dev, &vfe_dev->error_info.overflow_recover_irq_mask0,
          &vfe_dev->error_info.overflow_recover_irq_mask1);
    }
    atomic_set(&vfe_dev->error_info.overflow_state, OVERFLOW_DETECTED);
    pr_err("%s: Bus overflow detected: start recovery!\n", __func__);
  }

  rc = vfe_dev->hw_info->vfe_ops.axi_ops.halt(vfe_dev, halt_cmd->blocking_halt);
...
int msm_isp_axi_reset(struct vfe_device *vfe_dev,
  ...
  rc = vfe_dev->hw_info->vfe_ops.core_ops.reset_hw(vfe_dev, 0,
                                                   reset_cmd->blocking);

  for (i = 0, j = 0; j < axi_data->num_active_stream && i < MAX_NUM_STREAM;
       i++, j++) {
    stream_info = &axi_data->stream_info[i];
    ...
    for (bufq_id = 0; bufq_id < VFE_BUF_QUEUE_MAX; bufq_id++) {
      ...
      bufq = vfe_dev->buf_mgr->ops->get_bufq(vfe_dev->buf_mgr, bufq_handle);
      if (!bufq) {
      ...
      if (bufq->buf_type != ISP_SHARE_BUF) {
        ...
        vfe_dev->buf_mgr->ops->flush_buf(vfe_dev->buf_mgr, bufq_handle,
...
static int msm_isp_axi_update_cgc_override(
    struct vfe_device *vfe_dev,
    struct msm_vfe_axi_stream_cfg_cmd *stream_cfg_cmd, uint8_t cgc_override) {
  int i = 0, j = 0;
  struct msm_vfe_axi_stream *stream_info;
  struct msm_vfe_axi_shared_data *axi_data = &vfe_dev->axi_data;
  ...
  for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
    ...
    stream_info =
        &axi_data->stream_info[HANDLE_TO_IDX(stream_cfg_cmd->stream_handle[i])];
    for (j = 0; j < stream_info->num_planes; j++) {
      if (vfe_dev->hw_info->vfe_ops.axi_ops.update_cgc_override)
        vfe_dev->hw_info->vfe_ops.axi_ops.update_cgc_override(
            vfe_dev, stream_info->wm[j], cgc_override);
    }
  }
  return 0;
}
...
static int
msm_isp_start_axi_stream(struct vfe_device *vfe_dev,
                         struct msm_vfe_axi_stream_cfg_cmd *stream_cfg_cmd,
                         enum msm_isp_camif_update_state camif_update) {
  int i, rc = 0;
  ...
  for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
    ...
    if (SRC_TO_INTF(stream_info->stream_src) < VFE_SRC_MAX)
      src_state =
          axi_data->src_info[SRC_TO_INTF(stream_info->stream_src)].active;
    else {
    ...
    if (SRC_TO_INTF(stream_info->stream_src) >= VFE_RAW_0 &&
      ...
      vfe_dev->axi_data.src_info[SRC_TO_INTF(stream_info->stream_src)]
          .frame_id = 0;
      vfe_dev->axi_data.src_info[SRC_TO_INTF(stream_info->stream_src)].active =
          1;
    }
  }
  mutex_unlock(&vfe_dev->buf_mgr->lock);
  ...
  if (wait_for_complete) {
    ...
    rc = msm_isp_axi_wait_for_cfg_done(vfe_dev, camif_update, src_mask, 2);
...
static int
msm_isp_stop_axi_stream(struct vfe_device *vfe_dev,
                        struct msm_vfe_axi_stream_cfg_cmd *stream_cfg_cmd,
                        enum msm_isp_camif_update_state camif_update) {
  int i, rc = 0;
  ...
  struct msm_vfe_axi_stream *stream_info = NULL;
  struct msm_vfe_axi_shared_data *axi_data = &vfe_dev->axi_data;
  int ext_read = (axi_data->src_info[VFE_PIX_0].input_mux == EXTERNAL_READ);
  ...
  for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
    ...
    vfe_dev->hw_info->vfe_ops.axi_ops.cfg_framedrop(vfe_dev->vfe_base,
                                                    stream_info, 0, 0);
    pr_debug("%s, Stream 0x%x,\n", __func__, stream_info->stream_id);
     ...
     */
    if (!vfe_dev->axi_data.src_info[intf].active || ext_read ||
  ...
  if (src_mask) {
    ...
    if (rc < 0) {
      ...
      for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
        stream_info =
            &axi_data
                 ->stream_info[HANDLE_TO_IDX(stream_cfg_cmd->stream_handle[i])];
        stream_info->state = STOPPING;
        msm_isp_axi_stream_enable_cfg(vfe_dev, stream_info);
        vfe_dev->hw_info->vfe_ops.core_ops.reg_update(
            vfe_dev, SRC_TO_INTF(stream_info->stream_src));
        mutex_unlock(&vfe_dev->core_mutex);
        rc = msm_isp_axi_wait_for_cfg_done(vfe_dev, camif_update, src_mask, 1);
        mutex_lock(&vfe_dev->core_mutex);
        if (rc < 0)
...
int msm_isp_cfg_axi_stream(struct vfe_device *vfe_dev, void *arg) {
  ...
  if (stream_cfg_cmd->cmd == START_STREAM) {
    ...
    rc = msm_isp_stop_axi_stream(vfe_dev, stream_cfg_cmd, camif_update);
...
static int msm_isp_return_empty_buffer(struct vfe_device *vfe_dev,
  ...
  vfe_dev->buf_mgr->ops->buf_done(
      vfe_dev->buf_mgr, buf->bufq_handle, buf->buf_idx,
      &vfe_dev->axi_data.src_info[frame_src].time_stamp, frame_id,
      stream_info->runtime_output_format);

  memset(&error_event, 0, sizeof(error_event));
  error_event.frame_id = frame_id;
  error_event.timestamp = vfe_dev->axi_data.src_info[frame_src].time_stamp;
...
static int msm_isp_request_frame(struct vfe_device *vfe_dev,
                                 struct msm_vfe_axi_stream *stream_info,
                                 uint32_t user_stream_id, uint32_t frame_id) {
  struct msm_vfe_axi_stream_request_cmd stream_cfg_cmd;
  struct msm_vfe_frame_request_queue *queue_req;
  uint32_t pingpong_status;
  unsigned long flags;
  ...
  if (((vfe_dev->axi_data.src_info[VFE_PIX_0].active) &&
    ...
    rc = msm_isp_return_empty_buffer(vfe_dev, stream_info, user_stream_id,
                                     frame_id, frame_src);
    if (rc < 0)
  ...
  if ((frame_src == VFE_PIX_0) && !stream_info->undelivered_request_cnt &&
    ...
    pr_err("%s:%d vfe %d frame_id %d prev_pattern %x stream_id %x\n", __func__,
           ...
           stream_info->prev_framedrop_pattern, stream_info->stream_id);
  ...
  if (stream_info->undelivered_request_cnt == 1) {
    rc = msm_isp_cfg_ping_pong_address(vfe_dev, stream_info, VFE_PING_FLAG, 1,
                                       1);
    if (rc) {
      stream_info->undelivered_request_cnt--;
      spin_unlock_irqrestore(&stream_info->lock, flags);
    ...
    if (stream_info->stream_src < RDI_INTF_0 && vfe_dev->is_split &&
      ...
      vfe_dev->hw_info->vfe_ops.axi_ops.reload_wm(
          vfe_dev, vfe_dev->dual_vfe_res->vfe_base[ISP_VFE0],
          dual_vfe_res->wm_reload_mask[ISP_VFE0]);
      vfe_dev->hw_info->vfe_ops.axi_ops.reload_wm(
          vfe_dev, vfe_dev->vfe_base, dual_vfe_res->wm_reload_mask[ISP_VFE1]);
      dual_vfe_res->wm_reload_mask[ISP_VFE0] = 0;
      dual_vfe_res->wm_reload_mask[ISP_VFE1] = 0;
    } else if (!vfe_dev->is_split || (stream_info->stream_src >= RDI_INTF_0 &&
                                      stream_info->stream_src <= RDI_INTF_2)) {
      vfe_dev->hw_info->vfe_ops.axi_ops.reload_wm(
          vfe_dev, vfe_dev->vfe_base, dual_vfe_res->wm_reload_mask[vfe_id]);
    ...
    rc = msm_isp_cfg_ping_pong_address(vfe_dev, stream_info, pingpong_status, 1,
                                       1);
    if (rc) {
      stream_info->undelivered_request_cnt--;
      spin_unlock_irqrestore(&stream_info->lock, flags);
...
static int msm_isp_add_buf_queue(struct vfe_device *vfe_dev,
  ...
  stream_info->bufq_handle[bufq_id] = vfe_dev->buf_mgr->ops->get_bufq_handle(
      vfe_dev->buf_mgr, stream_info->session_id, stream_id);
...
int msm_isp_update_axi_stream(struct vfe_device *vfe_dev, void *arg) {
  ...
  for (i = 0; i < update_cmd->num_streams; i++) {
    ...
    stream_info =
        &axi_data->stream_info[HANDLE_TO_IDX(update_info->stream_handle)];
    if (SRC_TO_INTF(stream_info->stream_src) >= VFE_SRC_MAX)
      continue;
    ...
    if (stream_info->state != ACTIVE && stream_info->state != INACTIVE &&
      ...
      pr_err("%s: Invalid stream state %d, update cmd %d\n", __func__,
             stream_info->state, stream_info->stream_id);
      return -EINVAL;
    }
    if (update_cmd->update_type == UPDATE_STREAM_AXI_CONFIG &&
        atomic_read(
            &axi_data->axi_cfg_update[SRC_TO_INTF(stream_info->stream_src)])) {
  ...
  for (i = 0; i < update_cmd->num_streams; i++) {
    ...
    case DISABLE_STREAM_BUF_DIVERT:
      ...
      vfe_dev->buf_mgr->ops->flush_buf(
          vfe_dev->buf_mgr, stream_info->bufq_handle[VFE_BUF_QUEUE_DEFAULT],
          MSM_ISP_BUFFER_FLUSH_DIVERTED);
      break;
    case UPDATE_STREAM_FRAMEDROP_PATTERN: {
      uint32_t framedrop_period =
          msm_isp_get_framedrop_period(update_info->skip_pattern);
      if (stream_info->controllable_output) {
        pr_err("Controllable output streams does not support custom frame skip "
               "pattern\n");
        return -EINVAL;
      }
    ...
    case UPDATE_STREAM_SW_FRAME_DROP: {
      ...
      if (sw_skip_info->stream_src_mask != 0) {
        ...
        pr_debug("%s:%x sw skip type %x mode %d min %d max %d\n", __func__,
                 stream_info->stream_id, sw_skip_info->stats_type_mask,
                 sw_skip_info->skip_mode, sw_skip_info->min_frame_id,
    ...
    case UPDATE_STREAM_AXI_CONFIG: {
      for (j = 0; j < stream_info->num_planes; j++) {
        stream_info->plane_cfg[j] = update_info->plane_cfg[j];
      }
    ...
    case UPDATE_STREAM_REMOVE_BUFQ: {
      ...
      if (stream_info->state == ACTIVE) {
        ...
        mutex_unlock(&vfe_dev->core_mutex);
        rc = msm_isp_axi_wait_for_cfg_done(
            vfe_dev, NO_UPDATE, (1 << SRC_TO_INTF(stream_info->stream_src)), 2);
        mutex_lock(&vfe_dev->core_mutex);
        if (rc < 0)
...
void msm_isp_process_axi_irq(struct vfe_device *vfe_dev, uint32_t irq_status0,
  ...
  comp_mask =
      vfe_dev->hw_info->vfe_ops.axi_ops.get_comp_mask(irq_status0, irq_status1);
  wm_mask =
      vfe_dev->hw_info->vfe_ops.axi_ops.get_wm_mask(irq_status0, irq_status1);
  ...
  for (i = 0; i < axi_data->hw_info->num_comp_mask; i++) {
    ...
    if (comp_mask & (1 << i)) {
      ...
      rc = msm_isp_cfg_ping_pong_address(vfe_dev, stream_info, pingpong_status,
                                         valid_address, 1);

      spin_unlock_irqrestore(&stream_info->lock, flags);

      if (done_buf && !rc)
        msm_isp_process_done_buf(vfe_dev, stream_info, done_buf, ts);
    }
  }
...
void msm_isp_axi_disable_all_wm(struct vfe_device *vfe_dev) {
  ...
  for (i = 0; i < MAX_NUM_STREAM; i++) {
    ...
    for (j = 0; j < stream_info->num_planes; j++)
      vfe_dev->hw_info->vfe_ops.axi_ops.enable_wm(vfe_dev, stream_info->wm[j],
                                                  0);
  }
}
