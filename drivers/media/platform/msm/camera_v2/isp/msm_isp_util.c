...
int msm_isp_update_bandwidth(enum msm_isp_hw_client client, uint64_t ab,
                             uint64_t ib) {
  int i;
  struct msm_bus_paths *path;
  mutex_lock(&bandwidth_mgr_mutex);
  if (!isp_bandwidth_mgr.use_count || !isp_bandwidth_mgr.bus_client) {
    pr_err("%s:error bandwidth manager inactive use_cnt:%d bus_clnt:%d\n",
           __func__, isp_bandwidth_mgr.use_count, isp_bandwidth_mgr.bus_client);
    return -EINVAL;
  }
  ...
  path = &(msm_isp_bus_client_pdata
               .usecase[isp_bandwidth_mgr.bus_vector_active_idx]);
  path->vectors[0].ab = 0;
  path->vectors[0].ib = 0;
  for (i = 0; i < MAX_ISP_CLIENT; i++) {
    if (isp_bandwidth_mgr.client_info[i].active) {
      path->vectors[0].ab += isp_bandwidth_mgr.client_info[i].ab;
      path->vectors[0].ib += isp_bandwidth_mgr.client_info[i].ib;
    }
  }
  msm_bus_scale_client_update_request(isp_bandwidth_mgr.bus_client,
                                      isp_bandwidth_mgr.bus_vector_active_idx);
  /* Insert into circular buffer */
  msm_isp_update_req_history(isp_bandwidth_mgr.bus_client, path->vectors[0].ab,
...
void msm_isp_deinit_bandwidth_mgr(enum msm_isp_hw_client client) {
  ...
  msm_bus_scale_client_update_request(isp_bandwidth_mgr.bus_client, 0);
...
int msm_isp_get_clk_info(struct vfe_device *vfe_dev,
  ...
  for (i = 0; i < count; i++) {
    vfe_clk_info[i].clk_rate = (rates[i] == 0) ? (long)-1 : rates[i];
...
static inline int msm_isp_subscribe_event_mask(
    struct v4l2_fh *fh, struct v4l2_event_subscription *sub, int evt_mask_index,
...
static inline int
msm_isp_process_event_subscription(struct v4l2_fh *fh,
  ...
  for (evt_mask_index = ISP_EVENT_MASK_INDEX_STATS_NOTIFY;
    ...
    if (evt_mask & (1 << evt_mask_index)) {
      ...
      rc = msm_isp_subscribe_event_mask(fh, sub, evt_mask_index, evt_id,
                                        subscribe_flag);
      if (rc != 0) {
...
int msm_isp_subscribe_event(struct v4l2_subdev *sd, struct v4l2_fh *fh,
  ...
  if (rc == 0 && sub->type && sub->type < ISP_EVENT_BASE) {
    ...
    for (interface = 0; interface < VFE_SRC_MAX; interface++) {
      ...
      extra_rc = v4l2_event_subscribe(fh, &extra, MAX_ISP_V4l2_EVENTS, NULL);
      if (extra_rc)
        pr_err("cityman_vfe extra SOF|%u rc=%d\n", interface, extra_rc);
    }
    extra.type = ISP_EVENT_BUF_DONE;
    extra_rc = v4l2_event_subscribe(fh, &extra, MAX_ISP_V4l2_EVENTS, NULL);
    if (extra_rc)
      pr_err("cityman_vfe extra BUF_DONE rc=%d\n", extra_rc);
    for (stream = 0; stream < MAX_NUM_STREAM; stream++) {
      extra.type = ISP_EVENT_BUF_DIVERT + stream;
      extra_rc = v4l2_event_subscribe(fh, &extra, MAX_ISP_V4l2_EVENTS, NULL);
      if (extra_rc)
        pr_err("cityman_vfe extra DIVERT+%u rc=%d\n", stream, extra_rc);
    }
    pr_err("cityman_vfe extra SOF+BUF_DONE+DIVERT after mask 0x%x rc0=%d\n",
           sub->type, rc);
  }
  return rc;
}
...
static int msm_isp_get_clk_rates(struct vfe_device *vfe_dev,
  ...
  if (!of_node) {
    pr_err("%s %d failed: of_node = %pK\n", __func__, __LINE__, of_node);
    return -EINVAL;
  }
  rc = of_property_read_u32(of_node, "max-clk-nominal", &nominal);
  if (rc < 0 || !nominal) {
    pr_err("%s: nominal rate error\n", __func__);
    return -EINVAL;
  }

  rc = of_property_read_u32(of_node, "max-clk-turbo", &turbo);
  if (rc < 0 || !turbo) {
...
static int msm_isp_set_clk_rate(struct vfe_device *vfe_dev, long *rate) {
  ...
  long round_rate = clk_round_rate(vfe_dev->vfe_clk[clk_idx], *rate);
  if (round_rate < 0) {
    pr_err("%s: Invalid vfe clock rate\n", __func__);
    return round_rate;
  }
...
void msm_isp_fetch_engine_done_notify(
  ...
  fe_rd_done_event.frame_id = vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id;
  ...
  ISP_DBG("%s:VFE%d ISP_EVENT_FE_READ_DONE buf_idx %d\n", __func__,
          vfe_dev->pdev->id, fetch_engine_info->buf_idx);
...
int msm_isp_cfg_pix(struct vfe_device *vfe_dev,
  ...
  vfe_dev->axi_data.src_info[VFE_PIX_0].pixel_clock = input_cfg->input_pix_clk;
  if (vfe_dev->axi_data.src_info[VFE_PIX_0].pixel_clock != 600000000) {
    pr_err("cityman_vfe pixclk %ld -> 600000000\n",
  ...
  vfe_dev->hw_info->vfe_ops.core_ops.cfg_input_mux(vfe_dev,
                                                   &input_cfg->d.pix_cfg);
...
int msm_isp_cfg_rdi(struct vfe_device *vfe_dev,
  ...
  vfe_dev->hw_info->vfe_ops.core_ops.cfg_rdi_reg(vfe_dev, &input_cfg->d.rdi_cfg,
                                                 input_cfg->input_src);
  return rc;
}
...
static int msm_isp_proc_cmd_list_unlocked(struct vfe_device *vfe_dev,
                                          void *arg) {
  int rc = 0;
  struct msm_vfe_cfg_cmd_list *proc_cmd = (struct msm_vfe_cfg_cmd_list *)arg;
  struct msm_vfe_cfg_cmd_list cmd, cmd_next;

  if (!vfe_dev || !arg) {
    pr_err("%s:%d failed: vfe_dev %pK arg %pK", __func__, __LINE__, vfe_dev,
           arg);
    return -EINVAL;
  }
  ...
  while (cmd.next) {
    if (cmd.next_size != sizeof(struct msm_vfe_cfg_cmd_list)) {
      pr_err("%s:%d failed: next size %u != expected %zu\n", __func__, __LINE__,
             cmd.next_size, sizeof(struct msm_vfe_cfg_cmd_list));
      break;
    }
...
static void
msm_isp_compat_to_proc_cmd(struct msm_vfe_cfg_cmd2 *proc_cmd,
...
static long msm_isp_ioctl_unlocked(struct v4l2_subdev *sd, unsigned int cmd,
                                   void *arg) {
  long rc = 0;
  struct vfe_device *vfe_dev = v4l2_get_subdevdata(sd);
  ...
  if (!vfe_dev || !vfe_dev->vfe_base) {
    ...
    if (vfe_dev)
      pr_err("%s:%d failed %pK\n", __func__, __LINE__, vfe_dev->vfe_base);
    return -EINVAL;
  }
  ...
  case VIDIOC_MSM_ISP_CFG_STREAM: {
    ...
    if (c)
      pr_err("cityman_isp CFG_STREAM n=%u cmd=%u h0=0x%x\n", c->num_streams,
             c->cmd, c->num_streams ? c->stream_handle[0] : 0);
    mutex_lock(&vfe_dev->core_mutex);
  ...
  case VIDIOC_MSM_ISP_REG_UPDATE_CMD:
    if (arg) {
      ...
      vfe_dev->hw_info->vfe_ops.core_ops.reg_update(vfe_dev, frame_src);
    }
    break;
  ...
  default:
    pr_err_ratelimited("%s: Invalid ISP command %d nr=%u size=%u\n", __func__,
...
#ifdef CONFIG_COMPAT
static long msm_isp_ioctl_compat(struct v4l2_subdev *sd, unsigned int cmd,
                                 void *arg) {
  struct vfe_device *vfe_dev = v4l2_get_subdevdata(sd);
  long rc = 0;
  ...
  case VIDIOC_MSM_VFE_REG_CFG_COMPAT: {
    ...
    mutex_lock(&vfe_dev->realtime_mutex);
    msm_isp_compat_to_proc_cmd(&proc_cmd, (struct msm_vfe_cfg_cmd2_32 *)arg);
...
static int msm_isp_send_hw_cmd(struct vfe_device *vfe_dev,
  ...
  if ((reg_cfg_cmd->cmd_type != VFE_CFG_MASK) && (!cfg_data || !cmd_len)) {
    pr_err("%s:%d failed: cmd type %d cfg_data %pK cmd_len %d\n", __func__,
           __LINE__, reg_cfg_cmd->cmd_type, cfg_data, cmd_len);
    return -EINVAL;
  }
  ...
  case VFE_WRITE_MB: {
    if ((reg_cfg_cmd->u.rw_info.reg_offset >
        ...
        ((reg_cfg_cmd->u.rw_info.reg_offset + reg_cfg_cmd->u.rw_info.len) >
    ...
    if ((reg_cfg_cmd->u.rw_info.cmd_data_offset >
         (UINT_MAX - reg_cfg_cmd->u.rw_info.len)) ||
        ((reg_cfg_cmd->u.rw_info.cmd_data_offset + reg_cfg_cmd->u.rw_info.len) >
         cmd_len)) {
  ...
  case VFE_READ_DMI_64BIT: {
    if (reg_cfg_cmd->cmd_type == VFE_WRITE_DMI_64BIT ||
      ...
      if (((UINT_MAX - reg_cfg_cmd->u.dmi_info.hi_tbl_offset) <
           (reg_cfg_cmd->u.dmi_info.len - sizeof(uint32_t))) ||
          ((reg_cfg_cmd->u.dmi_info.hi_tbl_offset +
            reg_cfg_cmd->u.dmi_info.len - sizeof(uint32_t)) > cmd_len)) {
    ...
    if ((reg_cfg_cmd->u.dmi_info.lo_tbl_offset >
        ...
        ((reg_cfg_cmd->u.dmi_info.lo_tbl_offset + reg_cfg_cmd->u.dmi_info.len) >
         cmd_len)) {
  ...
  case VFE_CFG_MASK: {
    ...
    bool grab_lock;
    unsigned long flags;
    if ((UINT_MAX - sizeof(temp) < reg_cfg_cmd->u.mask_info.reg_offset) ||
        (resource_size(vfe_dev->vfe_mem) <
         reg_cfg_cmd->u.mask_info.reg_offset + sizeof(temp))) {
    ...
    msm_camera_io_w(temp,
                    vfe_dev->vfe_base + reg_cfg_cmd->u.mask_info.reg_offset);
    if (grab_lock)
  ...
  case VFE_WRITE_DMI_64BIT: {
    ...
    if (reg_cfg_cmd->cmd_type == VFE_WRITE_DMI_64BIT) {
      hi_tbl_ptr = cfg_data + reg_cfg_cmd->u.dmi_info.hi_tbl_offset / 4;
    }
    lo_tbl_ptr = cfg_data + reg_cfg_cmd->u.dmi_info.lo_tbl_offset / 4;
    if (reg_cfg_cmd->cmd_type == VFE_WRITE_DMI_64BIT)
    ...
    for (i = 0; i < reg_cfg_cmd->u.dmi_info.len / 4; i++) {
      ...
      } else if (reg_cfg_cmd->cmd_type == VFE_WRITE_DMI_64BIT) {
        lo_tbl_ptr++;
        hi_val = *hi_tbl_ptr;
        hi_tbl_ptr = hi_tbl_ptr + 2;
        msm_camera_io_w(hi_val,
                        vfe_dev->vfe_base + vfe_dev->hw_info->dmi_reg_offset);
      }
  ...
  case VFE_HW_UPDATE_LOCK: {
    uint32_t update_id = vfe_dev->axi_data.src_info[VFE_PIX_0].last_updt_frm_id;
    if (vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id != *cfg_data ||
        update_id == *cfg_data) {
      ISP_DBG("%s hw update lock failed acq %d, cur id %u, last id %u\n",
              __func__, *cfg_data,
              vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id, update_id);
      return -EINVAL;
    }
    break;
  }
  case VFE_HW_UPDATE_UNLOCK: {
    if (vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id != *cfg_data) {
      ISP_DBG("hw update across frame boundary,begin id %u, end id %d\n",
              *cfg_data, vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id);
    }
  ...
  case VFE_READ: {
    int i;
    uint32_t *data_ptr = cfg_data + reg_cfg_cmd->u.rw_info.cmd_data_offset / 4;
  ...
  case GET_CLK_RATES: {
    int rc = 0;
    struct msm_isp_clk_rates rates;
    struct msm_isp_clk_rates *user_data = (struct msm_isp_clk_rates *)cfg_data;
    if (cmd_len != sizeof(struct msm_isp_clk_rates)) {
      pr_err("%s:%d failed: invalid cmd len %u exp %zu\n", __func__, __LINE__,
             cmd_len, sizeof(struct msm_isp_clk_rates));
      return -EINVAL;
    }
  ...
  case SET_UB_POLICY: {
    ...
    if (cmd_len < sizeof(vfe_dev->vfe_ub_policy)) {
      pr_err("%s:%d failed: invalid cmd len %u exp %zu\n", __func__, __LINE__,
             cmd_len, sizeof(vfe_dev->vfe_ub_policy));
      return -EINVAL;
    }
...
int msm_isp_proc_cmd(struct vfe_device *vfe_dev, void *arg) {
  ...
  if (proc_cmd->num_cfg <= ARRAY_SIZE(cfg_cmd_onstack)) {
    ...
    reg_cfg_cmd = kmalloc(
        sizeof(struct msm_vfe_reg_cfg_cmd) * proc_cmd->num_cfg, GFP_KERNEL);
    if (!reg_cfg_cmd) {
  ...
  if (copy_from_user(reg_cfg_cmd, (void __user *)(proc_cmd->cfg_cmd),
  ...
  if (proc_cmd->cmd_len > 0) {
    ...
    if (copy_from_user(cfg_data, (void __user *)(proc_cmd->cfg_data),
                       proc_cmd->cmd_len)) {
      rc = -EFAULT;
      goto copy_cmd_failed;
    }
  ...
  for (i = 0; i < proc_cmd->num_cfg; i++)
    rc = msm_isp_send_hw_cmd(vfe_dev, &reg_cfg_cmd[i], cfg_data,
                             proc_cmd->cmd_len);

  if (copy_to_user(proc_cmd->cfg_data, cfg_data, proc_cmd->cmd_len)) {
    rc = -EFAULT;
    goto copy_cmd_failed;
  }
...
int msm_isp_send_event(struct vfe_device *vfe_dev, uint32_t event_type,
                       struct msm_isp_event_data *event_data) {
  struct v4l2_event isp_event;
  memset(&isp_event, 0, sizeof(struct v4l2_event));
  isp_event.id = 0;
  isp_event.type = event_type;
  memcpy(&isp_event.u.data[0], event_data, sizeof(struct msm_isp_event_data));
...
static void msm_isp_process_overflow_irq(struct vfe_device *vfe_dev,
  ...
  if (overflow_mask) {
    ...
    pr_err("cityman_vfe overflow mask=0x%x s0=0x%x s1=0x%x\n", overflow_mask,
...
void msm_isp_reset_burst_count_and_frame_drop(
  ...
  if (stream_info->stream_type == BURST_STREAM &&
    ...
    framedrop_period =
        msm_isp_get_framedrop_period(stream_info->frame_skip_pattern);
    stream_info->burst_frame_count =
        stream_info->init_frame_drop +
        (stream_info->num_burst_capture - 1) * framedrop_period + 1;
...
static void msm_isp_enqueue_tasklet_cmd(struct vfe_device *vfe_dev,
  ...
  vfe_dev->taskletq_idx = (vfe_dev->taskletq_idx + 1) % MSM_VFE_TASKLETQ_SIZE;
...
irqreturn_t msm_isp_process_irq(int irq_num, void *data) {
  ...
  vfe_dev->hw_info->vfe_ops.irq_ops.read_irq_status(vfe_dev, &irq_status0,
                                                    &irq_status1);
  ...
  msm_isp_process_overflow_irq(vfe_dev, &irq_status0, &irq_status1);

  vfe_dev->hw_info->vfe_ops.core_ops.get_error_mask(&error_mask0, &error_mask1);
  error_mask0 &= irq_status0;
  error_mask1 &= irq_status1;
  irq_status0 &= ~error_mask0;
  irq_status1 &= ~error_mask1;
  if (!vfe_dev->ignore_error && ((error_mask0 != 0) || (error_mask1 != 0)))
...
void msm_isp_do_tasklet(unsigned long data) {
  ...
  while (atomic_read(&vfe_dev->irq_cnt)) {
    ...
    irq_ops->process_halt_irq(vfe_dev, irq_status0, irq_status1);
    if (atomic_read(&vfe_dev->error_info.overflow_state) != NO_OVERFLOW) {
      pr_err("%s: Recovery in processing, Ignore IRQs!!!\n", __func__);
      continue;
    }
    msm_isp_process_error_info(vfe_dev);
    irq_ops->process_camif_irq(vfe_dev, irq_status0, irq_status1, &ts);
    irq_ops->process_stats_irq(vfe_dev, irq_status0, irq_status1, &ts);
    irq_ops->process_axi_irq(vfe_dev, irq_status0, irq_status1, &ts);
    irq_ops->process_reg_update(vfe_dev, irq_status0, irq_status1, &ts);
    irq_ops->process_epoch_irq(vfe_dev, irq_status0, irq_status1, &ts);
  }
}
  ...
  if (token) {
    ...
    if (!vfe_dev->buf_mgr || !vfe_dev->buf_mgr->ops ||
      ...
      pr_err("%s:%d] buf_mgr %pK active strms %d\n", __func__, __LINE__,
             vfe_dev->buf_mgr, vfe_dev->axi_data.num_active_stream);
      goto end;
    }
...
int msm_isp_open_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh) {
  ...
  if (vfe_dev->vfe_base) {
    pr_err("%s:%d invalid params cnt %d base %pK\n", __func__, __LINE__,
           vfe_dev->vfe_open_cnt, vfe_dev->vfe_base);
  ...
  memset(&vfe_dev->stats_data, 0, sizeof(struct msm_vfe_stats_shared_data));
  memset(&vfe_dev->error_info, 0, sizeof(vfe_dev->error_info));
  memset(&vfe_dev->fetch_engine_info, 0, sizeof(vfe_dev->fetch_engine_info));
...
void msm_isp_end_avtimer(void) { avcs_core_disable_power_collapse(0); }
#else
void msm_isp_end_avtimer(void) { pr_err("AV Timer is not supported\n"); }
#endif
...
int msm_isp_close_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh) {
  ...
  iommu_set_fault_handler(vfe_dev->buf_mgr->iommu_domain, NULL, NULL);
