...
static struct vfe_device *cityman_vfe_mid_dev;
static void cityman_vfe_camif_mid(struct work_struct *w) {
  static int n;
  struct vfe_device *vfe_dev = cityman_vfe_mid_dev;
  uint32_t st, extra;
  ...
  pr_err("cityman_vfe camif mid n=%d st=0x%x extra=0x%x cmd=0x%x cfg=0x%x\n", n,
         st, extra, msm_camera_io_r(vfe_dev->vfe_base + 0x2F4),
         msm_camera_io_r(vfe_dev->vfe_base + 0x2F8));
}
static DECLARE_DELAYED_WORK(cityman_vfe_camif_mid_w, cityman_vfe_camif_mid);
...
static uint8_t stats_pingpong_offset_map[] = {7, 8, 9, 10, 11, 12, 13, 14, 15};
...
#define VFE44_STATS_PING_PONG_BASE(idx, ping_pong)                             \
  (VFE44_STATS_BASE(idx) +                                                     \
   0x4 * (~(ping_pong >> (stats_pingpong_offset_map[idx])) & 0x1))
...
static int32_t msm_vfe44_init_dt_parms(struct vfe_device *vfe_dev,
  ...
  rc = of_property_read_u32(of_node, dt_parms->entries, &dt_entries);
  if (rc < 0 || !dt_entries) {
    pr_err("%s: NO QOS entries found\n", __func__);
    return -EINVAL;
  } else {
    dt_settings = kzalloc(sizeof(uint32_t) * dt_entries, GFP_KERNEL);
    if (!dt_settings) {
      pr_err("%s:%d No memory\n", __func__, __LINE__);
      return -ENOMEM;
    }
    dt_regs = kzalloc(sizeof(uint32_t) * dt_entries, GFP_KERNEL);
    if (!dt_regs) {
      pr_err("%s:%d No memory\n", __func__, __LINE__);
      kfree(dt_settings);
      return -ENOMEM;
    }
    rc = of_property_read_u32_array(of_node, dt_parms->regs, dt_regs,
                                    dt_entries);
    if (rc < 0) {
      ...
      if (dt_parms->settings) {
        rc = of_property_read_u32_array(of_node, dt_parms->settings,
        ...
        if (rc < 0) {
          ...
          for (i = 0; i < dt_entries; i++) {
            msm_camera_io_w(dt_settings[i], vfebase + dt_regs[i]);
          }
          kfree(dt_settings);
          kfree(dt_regs);
        }
...
static int msm_vfe44_init_hardware(struct vfe_device *vfe_dev) {
  ...
  if (vfe_dev->num_clk <= 0) {
    ...
    vfe_dev->vfe_clk =
        kzalloc(sizeof(struct clk *) * vfe_dev->num_clk, GFP_KERNEL);
    if (!vfe_dev->vfe_clk) {
      pr_err("%s:%d No memory\n", __func__, __LINE__);
      return -ENOMEM;
    }
  ...
  vfe_dev->vfe_base =
      ioremap(vfe_dev->vfe_mem->start, resource_size(vfe_dev->vfe_mem));
  if (!vfe_dev->vfe_base) {
    rc = -ENOMEM;
...
vfe_remap_failed:
  msm_cam_clk_enable(&vfe_dev->pdev->dev, msm_vfe44_clk_info, vfe_dev->vfe_clk,
                     vfe_dev->num_clk, 0);
clk_enable_failed:
  if (vfe_dev->fs_vfe)
...
static void msm_vfe44_process_input_irq(struct vfe_device *vfe_dev,
  ...
  if (irq_status0 & (1 << 24)) {
    ...
    msm_isp_fetch_engine_done_notify(vfe_dev, &vfe_dev->fetch_engine_info);
  }
...
static void msm_vfe44_process_violation_status(struct vfe_device *vfe_dev) {
...
static void msm_vfe44_process_error_status(struct vfe_device *vfe_dev) {
  ...
  if (error_status1 & (1 << 0)) {
    ...
    pr_err("cityman_vfe camif err cfg1c=0x%x in2e8=0x%x st=0x%x\n",
...
static void msm_vfe44_process_reg_update(struct vfe_device *vfe_dev,
  ...
  for (i = VFE_PIX_0; i <= VFE_RAW_2; i++) {
    if (shift_irq & BIT(i)) {
      reg_updated |= BIT(i);
      ISP_DBG("%s update_mask %x\n", __func__, (uint32_t)BIT(i));
      switch (i) {
      case VFE_PIX_0:
        for (j = 0; j < MAX_NUM_STREAM; j++) {
          stream_info = &vfe_dev->axi_data.stream_info[j];
          stream_info->prev_framedrop_pattern = stream_info->framedrop_pattern;
          stream_info->prev_framedrop_period = stream_info->framedrop_period;
        }
      ...
      if (atomic_read(&vfe_dev->axi_data.axi_cfg_update[i])) {
        ...
        if (atomic_read(&vfe_dev->axi_data.axi_cfg_update[i]) == 0)
          msm_isp_notify(vfe_dev, ISP_EVENT_STREAM_UPDATE_DONE, i, ts);
      }
    }
  }
...
static long msm_vfe44_reset_hardware(struct vfe_device *vfe_dev,
  ...
  if (blocking_call) {
    rc = wait_for_completion_timeout(&vfe_dev->reset_complete,
                                     msecs_to_jiffies(50));
    if (rc <= 0) {
...
static void msm_vfe44_axi_enable_wm(struct vfe_device *vfe_dev, uint8_t wm_idx,
                                    uint8_t enable) {
  uint32_t val;
  ...
  if (enable)
    ...
    val &= ~0x1;
  msm_camera_io_w_mb(val, vfe_dev->vfe_base + VFE44_WM_BASE(wm_idx));
}
...
static void
msm_vfe44_axi_cfg_comp_mask(struct vfe_device *vfe_dev,
  ...
  comp_mask |= (axi_data->composite_info[comp_mask_index].stream_composite_mask
                << (comp_mask_index * 8));
...
static void
msm_vfe44_axi_clear_comp_mask(struct vfe_device *vfe_dev,
                              struct msm_vfe_axi_stream *stream_info) {
  uint32_t comp_mask, comp_mask_index = stream_info->comp_mask_index;
  uint32_t irq_mask;
...
static void
msm_vfe44_axi_cfg_wm_irq_mask(struct vfe_device *vfe_dev,
                              struct msm_vfe_axi_stream *stream_info) {
  uint32_t irq_mask;
  irq_mask = msm_camera_io_r(vfe_dev->vfe_base + 0x28);
...
static void
msm_vfe44_axi_clear_wm_irq_mask(struct vfe_device *vfe_dev,
                                struct msm_vfe_axi_stream *stream_info) {
  uint32_t irq_mask;
  irq_mask = msm_camera_io_r(vfe_dev->vfe_base + 0x28);
  irq_mask &= ~(1 << (stream_info->wm[0] + 8));
...
static void msm_vfe44_cfg_framedrop(void __iomem *vfe_base,
  ...
  for (i = 0; i < stream_info->num_planes; i++) {
    msm_camera_io_w(framedrop_pattern,
                    vfe_base + VFE44_WM_BASE(stream_info->wm[i]) + 0x1C);
    temp = msm_camera_io_r(vfe_base + VFE44_WM_BASE(stream_info->wm[i]) + 0xC);
    temp &= 0xFFFFFF83;
...
static void msm_vfe44_clear_framedrop(struct vfe_device *vfe_dev,
  ...
  uint32_t i;
  for (i = 0; i < stream_info->num_planes; i++)
    msm_camera_io_w(0, vfe_dev->vfe_base + VFE44_WM_BASE(stream_info->wm[i]) +
                           0x1C);
}
...
static int32_t
msm_vfe44_convert_io_fmt_to_reg(enum msm_isp_pack_fmt pack_format,
                                uint32_t *pack_reg) {
  int rc = 0;
...
static int32_t msm_vfe44_cfg_io_format(struct vfe_device *vfe_dev,
  ...
  if ((stream_src < RDI_INTF_0) &&
      (vfe_dev->axi_data.src_info[VFE_PIX_0].input_mux == EXTERNAL_READ)) {
    ...
    read_pack_fmt = msm_isp_get_pack_format(
        vfe_dev->axi_data.src_info[VFE_PIX_0].input_format);
    rc = msm_vfe44_convert_io_fmt_to_reg(read_pack_fmt, &read_pack_reg);
    if (rc < 0) {
...
static int msm_vfe44_fetch_engine_start(struct vfe_device *vfe_dev, void *arg) {
  int rc = 0;
  ...
  rc = vfe_dev->buf_mgr->ops->get_buf_by_index(vfe_dev->buf_mgr, bufq_handle,
                                               fe_cfg->buf_idx, &buf);
  if (rc < 0) {
...
static void msm_vfe44_cfg_fetch_engine(struct vfe_device *vfe_dev,
  ...
  if (pix_cfg->input_mux == EXTERNAL_READ) {
    ...
    pr_debug("%s: fetch_dbg wd x ht buf = %d x %d, fe = %d x %d\n", __func__,
             fe_cfg->buf_width, fe_cfg->buf_height, fe_cfg->fetch_width,
             fe_cfg->fetch_height);

    vfe_dev->hw_info->vfe_ops.axi_ops.update_cgc_override(
        vfe_dev, VFE44_BUS_RD_CGC_OVERRIDE_BIT, 1);
...
static void msm_vfe44_cfg_camif(struct vfe_device *vfe_dev,
  ...
  msm_camera_io_w(first_pixel << 16 | last_pixel, vfe_dev->vfe_base + 0x304);
...
static void
msm_vfe44_update_camif_state(struct vfe_device *vfe_dev,
  ...
  if (update_state == ENABLE_CAMIF) {
    ...
    pr_err("cityman_vfe camif irq mask0=0x%x (EOF unmasked)\n", val);
    msm_camera_io_w_mb(0x140000, vfe_dev->vfe_base + 0x318);
    pr_err("cityman_vfe camif epoch0=20\n");

    bus_en =
        ((vfe_dev->axi_data.src_info[VFE_PIX_0].raw_stream_count > 0) ? 1 : 0);
    vfe_en =
        ((vfe_dev->axi_data.src_info[VFE_PIX_0].pix_stream_count > 0) ? 1 : 0);
    val = msm_camera_io_r(vfe_dev->vfe_base + 0x2F8);
    val &= 0xFFFFFF3F;
...
static void msm_vfe44_cfg_rdi_reg(struct vfe_device *vfe_dev,
                                  struct msm_vfe_rdi_cfg *rdi_cfg,
                                  enum msm_vfe_input_src input_src) {
  uint8_t rdi = input_src - VFE_RAW_0;
  uint32_t rdi_reg_cfg;
  rdi_reg_cfg = msm_camera_io_r(vfe_dev->vfe_base + VFE44_RDI_BASE(0));
  rdi_reg_cfg &= ~(BIT(16 + rdi));
  rdi_reg_cfg |= rdi_cfg->frame_based << (16 + rdi);
  msm_camera_io_w(rdi_reg_cfg, vfe_dev->vfe_base + VFE44_RDI_BASE(0));

  rdi_reg_cfg = msm_camera_io_r(vfe_dev->vfe_base + VFE44_RDI_BASE(rdi));
  rdi_reg_cfg &= 0x70003;
  rdi_reg_cfg |= (rdi * 3) << 28 | rdi_cfg->cid << 4 | 0x4;
  msm_camera_io_w(rdi_reg_cfg, vfe_dev->vfe_base + VFE44_RDI_BASE(rdi));
}

static void msm_vfe44_axi_cfg_wm_reg(struct vfe_device *vfe_dev,
                                     struct msm_vfe_axi_stream *stream_info,
                                     uint8_t plane_idx) {
  uint32_t val;
  ...
  if (!stream_info->frame_based) {
    ...
    val = ((msm_isp_cal_word_per_line(
                ...
                stream_info->plane_cfg[plane_idx].output_width) +
            1) /
               2 -
           1) << 16 |
    ...
    val = val << 2 |
          msm_isp_cal_word_per_line(
              stream_info->output_format,
              stream_info->plane_cfg[plane_idx].output_stride)
              << 16 |
          VFE44_BURST_LEN;
  ...
  msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + wm_base + 0x20);
  /* TD: Add IRQ subsample pattern */
}

static void msm_vfe44_axi_clear_wm_reg(struct vfe_device *vfe_dev,
                                       struct msm_vfe_axi_stream *stream_info,
                                       uint8_t plane_idx) {
  uint32_t val = 0;
  uint32_t wm_base = VFE44_WM_BASE(stream_info->wm[plane_idx]);
...
static void
msm_vfe44_axi_cfg_wm_xbar_reg(struct vfe_device *vfe_dev,
                              struct msm_vfe_axi_stream *stream_info,
                              uint8_t plane_idx) {
  struct msm_vfe_axi_plane_cfg *plane_cfg = &stream_info->plane_cfg[plane_idx];
  ...
  xbar_reg_cfg = msm_camera_io_r(vfe_dev->vfe_base + VFE44_XBAR_BASE(wm));
  xbar_reg_cfg &= ~(0xFFFF << VFE44_XBAR_SHIFT(wm));
  xbar_reg_cfg |= (xbar_cfg << VFE44_XBAR_SHIFT(wm));
  msm_camera_io_w(xbar_reg_cfg, vfe_dev->vfe_base + VFE44_XBAR_BASE(wm));
}

static void
msm_vfe44_axi_clear_wm_xbar_reg(struct vfe_device *vfe_dev,
                                struct msm_vfe_axi_stream *stream_info,
                                uint8_t plane_idx) {
  uint8_t wm = stream_info->wm[plane_idx];
  uint32_t xbar_reg_cfg = 0;
...
static void msm_vfe44_cfg_axi_ub_equal_default(struct vfe_device *vfe_dev) {
  int i;
  ...
  prop_size =
      MSM_ISP44_TOTAL_IMAGE_UB - axi_data->hw_info->min_wm_ub * num_used_wms;
  for (i = 0; i < axi_data->hw_info->num_wm; i++) {
    if (axi_data->free_wm[i]) {
      delta = (uint64_t)axi_data->wm_image_size[i] * (uint64_t)prop_size;
      do_div(delta, total_image_size);
      wm_ub_size = axi_data->hw_info->min_wm_ub + (uint32_t)delta;
      msm_camera_io_w(ub_offset << 16 | (wm_ub_size - 1),
                      vfe_dev->vfe_base + VFE44_WM_BASE(i) + 0x10);
      ub_offset += wm_ub_size;
    } else
      msm_camera_io_w(0, vfe_dev->vfe_base + VFE44_WM_BASE(i) + 0x10);
  }
}

static void msm_vfe44_cfg_axi_ub_equal_slicing(struct vfe_device *vfe_dev) {
  int i;
  uint32_t ub_offset = 0;
  struct msm_vfe_axi_shared_data *axi_data = &vfe_dev->axi_data;
  uint32_t ub_equal_slice =
      MSM_ISP44_TOTAL_IMAGE_UB / axi_data->hw_info->num_wm;
  for (i = 0; i < axi_data->hw_info->num_wm; i++) {
...
static void msm_vfe44_read_wm_ping_pong_addr(struct vfe_device *vfe_dev) {
  msm_camera_io_dump_2(vfe_dev->vfe_base + (VFE44_WM_BASE(0) & 0xFFFFFFF0),
                       0x100);
}

static void msm_vfe44_update_ping_pong_addr(void __iomem *vfe_base,
                                            uint8_t wm_idx,
                                            uint32_t pingpong_status,
                                            dma_addr_t paddr) {
  uint32_t paddr32 = (paddr & 0xFFFFFFFF);
  msm_camera_io_w(paddr32,
                  vfe_base + VFE44_PING_PONG_BASE(wm_idx, pingpong_status));
}

static int msm_vfe44_axi_halt(struct vfe_device *vfe_dev, uint32_t blocking) {
  int rc = 0;
  enum msm_vfe_input_src i;
  ...
  if (blocking) {
    ...
    rc = wait_for_completion_timeout(&vfe_dev->halt_complete,
...
static int msm_vfe44_axi_restart(struct vfe_device *vfe_dev, uint32_t blocking,
...
static uint32_t msm_vfe44_get_wm_mask(uint32_t irq_status0,
                                      uint32_t irq_status1) {
  return (irq_status0 >> 8) & 0x7F;
}

static uint32_t msm_vfe44_get_comp_mask(uint32_t irq_status0,
                                        uint32_t irq_status1) {
  return (irq_status0 >> 25) & 0xF;
}

static uint32_t msm_vfe44_get_pingpong_status(struct vfe_device *vfe_dev) {
...
static int
msm_vfe44_stats_check_streams(struct msm_vfe_stats_stream *stream_info) {
...
static void msm_vfe44_stats_cfg_comp_mask(struct vfe_device *vfe_dev,
                                          uint32_t stats_mask,
                                          uint8_t request_comp_index,
                                          uint8_t enable) {
  ...
  if (request_comp_index >= MAX_NUM_STATS_COMP_MASK) {
    pr_err("%s: num of comp masks %d exceed max %d\n", __func__,
           request_comp_index, MAX_NUM_STATS_COMP_MASK);
    return;
  }
  ...
  if (enable) {
    ...
    atomic_set(stats_comp_mask, stats_mask | atomic_read(stats_comp_mask));
  } else {
    if (!(atomic_read(stats_comp_mask) & stats_mask))
      return;
    if (stats_mask & (1 << STATS_IDX_BF_SCALE) &&
        atomic_read(stats_comp_mask) & (1 << STATS_IDX_BF_SCALE))
      atomic_set(stats_comp_mask,
                 ~(1 << STATS_IDX_BF_SCALE) & atomic_read(stats_comp_mask));
  ...
  ISP_DBG("%s: comp_mask_reg: %x comp mask0 %x mask1: %x\n", __func__,
          comp_mask_reg, atomic_read(&stats_data->stats_comp_mask[0]),
...
static void
msm_vfe44_stats_cfg_wm_irq_mask(struct vfe_device *vfe_dev,
...
static void
msm_vfe44_stats_clear_wm_irq_mask(struct vfe_device *vfe_dev,
                                  struct msm_vfe_stats_stream *stream_info) {
  uint32_t irq_mask;
  irq_mask = msm_camera_io_r(vfe_dev->vfe_base + 0x28);
  irq_mask &= ~(1 << (STATS_IDX(stream_info->stream_handle) + 15));
...
static void
msm_vfe44_stats_cfg_wm_reg(struct vfe_device *vfe_dev,
                           struct msm_vfe_stats_stream *stream_info) {
  ...
  /*WR_IRQ_SUBSAMPLE_PATTERN*/
  msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + stats_base + 0x14);
}

static void
msm_vfe44_stats_clear_wm_reg(struct vfe_device *vfe_dev,
                             struct msm_vfe_stats_stream *stream_info) {
  uint32_t val = 0;
  int stats_idx = STATS_IDX(stream_info->stream_handle);
  uint32_t stats_base = VFE44_STATS_BASE(stats_idx);
...
static void msm_vfe44_stats_cfg_ub(struct vfe_device *vfe_dev) {
  ...
  for (i = 0; i < VFE44_NUM_STATS_TYPE; i++) {
    ub_offset -= ub_size[i];
    msm_camera_io_w(VFE44_STATS_BURST_LEN << 30 | ub_offset << 16 |
                        (ub_size[i] - 1),
...
static bool msm_vfe44_is_module_cfg_lock_needed(uint32_t reg_offset) {
  if (reg_offset == 0x18)
    return true;
  else
    return false;
...
static void msm_vfe44_stats_update_ping_pong_addr(
  ...
  msm_camera_io_w(paddr32, vfe_base + VFE44_STATS_PING_PONG_BASE(
                                          stats_idx, pingpong_status));
}

static uint32_t msm_vfe44_stats_get_wm_mask(uint32_t irq_status0,
                                            uint32_t irq_status1) {
  return (irq_status0 >> 15) & 0x1FF;
}

static uint32_t msm_vfe44_stats_get_comp_mask(uint32_t irq_status0,
                                              uint32_t irq_status1) {
  return (irq_status0 >> 29) & 0x3;
}

static uint32_t msm_vfe44_stats_get_frame_id(struct vfe_device *vfe_dev) {
...
static int msm_vfe44_get_platform_data(struct vfe_device *vfe_dev) {
  int rc = 0;
  vfe_dev->vfe_mem =
      platform_get_resource_byname(vfe_dev->pdev, IORESOURCE_MEM, "vfe");
  if (!vfe_dev->vfe_mem) {
  ...
  vfe_dev->vfe_vbif_mem =
      platform_get_resource_byname(vfe_dev->pdev, IORESOURCE_MEM, "vfe_vbif");
  ...
  vfe_dev->vfe_irq =
      platform_get_resource_byname(vfe_dev->pdev, IORESOURCE_IRQ, "vfe");
  if (!vfe_dev->vfe_irq) {
...
static void msm_vfe44_get_error_mask(uint32_t *error_mask0,
...
static struct msm_vfe_stats_hardware_info msm_vfe44_stats_hw_info = {
    .stats_capability_mask = 1 << MSM_ISP_STATS_BE | 1 << MSM_ISP_STATS_BF |
            ...
            .axi_ops =
                {
                    ...
                    .read_wm_ping_pong_addr = msm_vfe44_read_wm_ping_pong_addr,
                    .update_ping_pong_addr = msm_vfe44_update_ping_pong_addr,
                    ...
                    .update_cgc_override = msm_vfe44_axi_update_cgc_override,
                },
            .core_ops =
                {
            ...
            .stats_ops =
                {
                    ...
                    .get_pingpong_status = msm_vfe44_get_pingpong_status,
                    .update_cgc_override = msm_vfe44_stats_update_cgc_override,
                },
        },
