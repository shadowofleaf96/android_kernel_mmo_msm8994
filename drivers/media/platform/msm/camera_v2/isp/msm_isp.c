...
static ssize_t vfe_debugfs_statistics_read(struct file *t_file, char *t_char,
  ...
  struct vfe_device *vfe_dev = (struct vfe_device *)t_file->private_data;
  ...
  return simple_read_from_buffer(t_char, t_size_t, t_loff_t, stat_line,
                                 strlen(stat_line));
}

static ssize_t vfe_debugfs_statistics_write(struct file *t_file,
                                            const char *t_char, size_t t_size_t,
                                            loff_t *t_loff_t) {
  struct vfe_device *vfe_dev = (struct vfe_device *)t_file->private_data;
  struct msm_isp_statistics *stats = vfe_dev->stats;
...
static ssize_t bw_history_read(struct file *t_file, char *t_char,
  ...
  for (i = 0; i < MAX_DEPTH_BW_REQ_HISTORY; i++) {
    snprintf(
        ...
        isp_req_hist[i].client_info[0].ab, isp_req_hist[i].client_info[0].ib,
        isp_req_hist[i].client_info[1].active,
        isp_req_hist[i].client_info[1].ab, isp_req_hist[i].client_info[1].ib,
        isp_req_hist[i].client_info[2].active,
        isp_req_hist[i].client_info[2].ab, isp_req_hist[i].client_info[2].ib);
    strlcat(out_buffer, line_buffer, sizeof(bw_request_history_buff));
  }
  return simple_read_from_buffer(t_char, t_size_t, t_loff_t, out_buffer,
                                 strlen(out_buffer));
}

static ssize_t bw_history_write(struct file *t_file, const char *t_char,
                                size_t t_size_t, loff_t *t_loff_t) {
  struct msm_isp_bw_req_info *isp_req_hist =
      (struct msm_isp_bw_req_info *)t_file->private_data;
...
static ssize_t ub_info_read(struct file *t_file, char *t_char, size_t t_size_t,
                            loff_t *t_loff_t) {
  int i;
  ...
  strlcat(out_buffer, line_buffer, sizeof(ub_info_buffer));
...
static ssize_t ub_info_write(struct file *t_file, const char *t_char,
                             size_t t_size_t, loff_t *t_loff_t) {
  struct vfe_device *vfe_dev = (struct vfe_device *)t_file->private_data;
  struct msm_isp_ub_info *ub_info = vfe_dev->ub_info;
...
static int msm_isp_enable_debugfs(struct vfe_device *vfe_dev,
  ...
  if (!debugfs_base)
    return -ENOMEM;
  if (!debugfs_create_file("stats", S_IRUGO | S_IWUSR, debugfs_base, vfe_dev,
                           &vfe_debugfs_error))
    return -ENOMEM;

  if (!debugfs_create_file("bw_req_history", S_IRUGO | S_IWUSR, debugfs_base,
                           isp_req_hist, &bw_history_ops))
    return -ENOMEM;

  if (!debugfs_create_file("ub_info", S_IRUGO | S_IWUSR, debugfs_base, vfe_dev,
                           &ub_info_ops))
    return -ENOMEM;
...
void msm_isp_update_req_history(uint32_t client, uint64_t ab, uint64_t ib,
                                struct msm_isp_bandwidth_info *client_info,
                                unsigned long long ts) {
  int i;

  spin_lock(&req_history_lock);
  msm_isp_bw_request_history[msm_isp_bw_request_history_idx].client = client;
  msm_isp_bw_request_history[msm_isp_bw_request_history_idx].timestamp = ts;
  msm_isp_bw_request_history[msm_isp_bw_request_history_idx].total_ab = ab;
  msm_isp_bw_request_history[msm_isp_bw_request_history_idx].total_ib = ib;

  for (i = 0; i < MAX_ISP_CLIENT; i++) {
    msm_isp_bw_request_history[msm_isp_bw_request_history_idx]
        .client_info[i]
        .active = client_info[i].active;
    msm_isp_bw_request_history[msm_isp_bw_request_history_idx]
        .client_info[i]
        .ab = client_info[i].ab;
    msm_isp_bw_request_history[msm_isp_bw_request_history_idx]
  ...
  msm_isp_bw_request_history_idx =
      (msm_isp_bw_request_history_idx + 1) % MAX_DEPTH_BW_REQ_HISTORY;
...
static long msm_isp_dqevent(struct file *file, struct v4l2_fh *vfh, void *arg) {
  ...
  if (is_compat_task()) {
    ...
    if (rc)
      return rc;
    event_data = (struct msm_isp_event_data *)isp_event.u.data;
    isp_event_user = (struct v4l2_event *)arg;
    memcpy(isp_event_user, &isp_event, sizeof(*isp_event_user));
    event_data32 = (struct msm_isp_event_data32 *)isp_event_user->u.data;
    memset(event_data32, 0, sizeof(struct msm_isp_event_data32));
    event_data32->timestamp.tv_sec = event_data->timestamp.tv_sec;
    event_data32->timestamp.tv_usec = event_data->timestamp.tv_usec;
    event_data32->mono_timestamp.tv_sec = event_data->mono_timestamp.tv_sec;
    event_data32->mono_timestamp.tv_usec = event_data->mono_timestamp.tv_usec;
    event_data32->frame_id = event_data->frame_id;
    memcpy(&(event_data32->u), &(event_data->u), sizeof(event_data32->u));
  } else {
    rc = v4l2_event_dequeue(vfh, arg, file->f_flags & O_NONBLOCK);
  }
  return rc;
}
#else
...
static long msm_isp_subdev_do_ioctl(struct file *file, unsigned int cmd,
                                    void *arg) {
...
static int vfe_probe(struct platform_device *pdev) {
  ...
  if (pdev->dev.of_node) {
    ...
    vfe_dev->hw_info = (struct msm_vfe_hardware_info *)match_dev->data;
  } else {
    vfe_dev->hw_info =
        (struct msm_vfe_hardware_info *)platform_get_device_id(pdev)
            ->driver_data;
  }
  ...
  vfe_dev->dual_vfe_res->axi_data[vfe_dev->pdev->id] = &vfe_dev->axi_data;
  vfe_dev->dual_vfe_res->stats_data[vfe_dev->pdev->id] = &vfe_dev->stats_data;
  ...
  tasklet_init(&vfe_dev->vfe_tasklet, msm_isp_do_tasklet,
               (unsigned long)vfe_dev);
  ...
  vfe_dev->subdev.sd.internal_ops = vfe_dev->hw_info->subdev_internal_ops;
  snprintf(vfe_dev->subdev.sd.name, ARRAY_SIZE(vfe_dev->subdev.sd.name), "vfe");
  ...
  v4l2_subdev_notify(&vfe_dev->subdev.sd, MSM_SD_NOTIFY_REQ_CB, &vfe_vb2_ops);
  rc = msm_isp_create_isp_buf_mgr(vfe_dev->buf_mgr, &vfe_vb2_ops, &vfe_layout);
  if (rc < 0) {
  ...
  if (vfe_dev->hw_info->num_iommu_secure_ctx) {
    ...
    rc = msm_isp_create_secure_domain(vfe_dev->buf_mgr, &vfe_secure_layout);
    if (rc < 0) {
  ...
  vfe_dev->buf_mgr->ops->register_ctx(
      vfe_dev->buf_mgr, &vfe_dev->iommu_ctx[0], &vfe_dev->iommu_secure_ctx[0],
      vfe_dev->hw_info->num_iommu_ctx, vfe_dev->hw_info->num_iommu_secure_ctx);
