...
static int32_t msm_camera_get_power_settimgs_from_sensor_lib(
  ...
  if ((NULL == power_info->power_setting) ||
    ...
    power_info->power_setting = kzalloc(sizeof(*ps) * size, GFP_KERNEL);
    if (!power_info->power_setting) {
      pr_err("%s failed %d\n", __func__, __LINE__);
      rc = -ENOMEM;
      goto FAILED_1;
    }
    memcpy(power_info->power_setting, power_setting_array->power_setting,
           sizeof(*ps) * size);
  ...
  power_info->power_down_setting = kzalloc(sizeof(*ps) * size, GFP_KERNEL);
  if (!power_info->power_down_setting) {
    pr_err("%s failed %d\n", __func__, __LINE__);
    goto FREE_UP;
  }
  memcpy(power_info->power_down_setting, ps, sizeof(*ps) * size);
  ...
  if (need_reverse) {
    ...
    for (c = 0; c < size / 2; c++) {
      ...
      power_info->power_down_setting[c] = power_info->power_down_setting[end];
      power_info->power_down_setting[end] = power_down_setting_t;
      end--;
    }
  }
...
static int32_t msm_sensor_get_dt_data(struct device_node *of_node,
  ...
  s_ctrl->sensordata =
      kzalloc(sizeof(struct msm_camera_sensor_board_info), GFP_KERNEL);
  ...
  CDBG("%s qcom,sensor-name %s, rc %d\n", __func__, sensordata->sensor_name,
       rc);
  if (rc < 0) {
    pr_err("%s failed %d\n", __func__, __LINE__);
  ...
  rc =
      of_property_read_u32(of_node, "qcom,cci-master", &s_ctrl->cci_i2c_master);
  CDBG("%s qcom,cci-master %d, rc %d\n", __func__, s_ctrl->cci_i2c_master, rc);
  if (rc < 0) {
  ...
  s_ctrl->set_mclk_23880000 =
      of_property_read_bool(of_node, "qcom,mclk-23880000");
  ...
  rc = msm_camera_get_dt_vreg_data(of_node, &sensordata->power_info.cam_vreg,
                                   &sensordata->power_info.num_vreg);
  if (rc < 0)
    goto FREE_CSI;

  rc = msm_camera_get_dt_power_setting_data(
      of_node, sensordata->power_info.cam_vreg, sensordata->power_info.num_vreg,
  ...
  rc = msm_camera_get_power_settimgs_from_sensor_lib(
      &sensordata->power_info, &s_ctrl->power_setting_array);
  if (rc < 0) {
    pr_err("%s failed %d\n", __func__, __LINE__);
    goto FREE_VREG;
  }
  ...
  if (gpio_array_size) {
    ...
    rc = msm_camera_get_dt_gpio_set_tbl(of_node, gconf, gpio_array,
                                        gpio_array_size);
    if (rc < 0) {
      pr_err("%s failed %d\n", __func__, __LINE__);
  ...
  rc = msm_sensor_get_dt_actuator_data(of_node, &sensordata->actuator_info);
  if (rc < 0) {
    pr_err("%s failed %d\n", __func__, __LINE__);
  ...
  sensordata->slave_info =
      kzalloc(sizeof(struct msm_camera_slave_info), GFP_KERNEL);
  ...
  rc = of_property_read_u32_array(of_node, "qcom,slave-id", id_info, count);
  if (rc < 0) {
    pr_err("%s failed %d\n", __func__, __LINE__);
    goto FREE_SLAVE_INFO;
  }
  ...
  CDBG("%s:%d slave addr 0x%x sensor reg 0x%x id 0x%x mask 0x%x\n", __func__,
       __LINE__, slave_info->sensor_slave_addr, slave_info->sensor_id_reg_addr,
...
static void msm_sensor_misc_regulator(struct msm_sensor_ctrl_t *sctrl,
                                      uint32_t enable) {
  int32_t rc = 0;
  if (enable) {
    ...
    if (sctrl->misc_regulator) {
      rc =
          rpm_regulator_set_mode(sctrl->misc_regulator, RPM_REGULATOR_MODE_HPM);
      if (rc < 0) {
    ...
    if (sctrl->misc_regulator) {
      rc = rpm_regulator_set_mode((struct rpm_regulator *)sctrl->misc_regulator,
...
int msm_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl) {
  ...
  if (!power_info || !sensor_i2c_client) {
    pr_err("%s:%d failed: power_info %pK sensor_i2c_client %pK\n", __func__,
           __LINE__, power_info, sensor_i2c_client);
    return -EINVAL;
  }
...
int msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl) {
  ...
  if (!power_info || !sensor_i2c_client || !slave_info || !sensor_name) {
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl) {
  ...
  rc = sensor_i2c_client->i2c_func_tbl->i2c_read(
      sensor_i2c_client, slave_info->sensor_id_reg_addr, &chipid,
...
static long msm_sensor_subdev_ioctl(struct v4l2_subdev *sd, unsigned int cmd,
                                    void *arg) {
  int rc = 0;
...
#ifdef CONFIG_COMPAT
static long msm_sensor_subdev_do_ioctl(struct file *file, unsigned int cmd,
                                       void *arg) {
...
long msm_sensor_subdev_fops_ioctl(struct file *file, unsigned int cmd,
                                  unsigned long arg) {
...
int msm_sensor_config32(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp) {
  ...
  case CFG_GET_SENSOR_INFO:
    memcpy(cdata->cfg.sensor_info.sensor_name, s_ctrl->sensordata->sensor_name,
    ...
    cdata->cfg.sensor_info.position = s_ctrl->sensordata->sensor_info->position;
  ...
  case CFG_WRITE_I2C_ARRAY: {
    ...
    if (copy_from_user(&conf_array32, (void *)compat_ptr(cdata->cfg.setting),
    ...
    if (!conf_array.size || conf_array.size > I2C_REG_DATA_MAX) {
      pr_err("%s:%d failed\n", __func__, __LINE__);
      rc = -EFAULT;
      break;
    }
    ...
    reg_setting =
        kzalloc(conf_array.size * (sizeof(struct msm_camera_i2c_reg_array)),
                GFP_KERNEL);
    if (!reg_setting) {
      pr_err("%s:%d failed\n", __func__, __LINE__);
      rc = -ENOMEM;
  ...
  case CFG_SLAVE_READ_I2C: {
    ...
    read_config_ptr =
        (struct msm_camera_i2c_read_config *)compat_ptr(cdata->cfg.setting);
    ...
    CDBG("%s:slave_addr=0x%x reg_addr=0x%x, data_type=%d\n", __func__,
         read_config.slave_addr, read_config.reg_addr, read_config.data_type);
    if (s_ctrl->sensor_i2c_client->cci_client) {
      orig_slave_addr = s_ctrl->sensor_i2c_client->cci_client->sid;
      s_ctrl->sensor_i2c_client->cci_client->sid = read_slave_addr >> 1;
    } else if (s_ctrl->sensor_i2c_client->client) {
      orig_slave_addr = s_ctrl->sensor_i2c_client->client->addr;
      s_ctrl->sensor_i2c_client->client->addr = read_slave_addr >> 1;
    } else {
      pr_err("%s: error: no i2c/cci client found.", __func__);
      rc = -EFAULT;
      break;
    }
    CDBG("%s:orig_slave_addr=0x%x, new_slave_addr=0x%x", __func__,
         orig_slave_addr, read_slave_addr >> 1);
  ...
  case CFG_WRITE_I2C_SEQ_ARRAY: {
    ...
    if (!conf_array.size || conf_array.size > I2C_SEQ_REG_DATA_MAX) {
      pr_err("%s:%d failed\n", __func__, __LINE__);
      rc = -EFAULT;
      break;
    }
    ...
    reg_setting =
        kzalloc(conf_array.size * (sizeof(struct msm_camera_i2c_seq_reg_array)),
                GFP_KERNEL);
    if (!reg_setting) {
      pr_err("%s:%d failed\n", __func__, __LINE__);
      rc = -ENOMEM;
    ...
    conf_array.reg_setting = reg_setting;
    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_seq_table(
        s_ctrl->sensor_i2c_client, &conf_array);
    kfree(reg_setting);
    break;
  }
  ...
  case CFG_SET_STOP_STREAM_SETTING: {
    ...
    stop_setting->reg_setting =
        kzalloc(stop_setting->size * (sizeof(struct msm_camera_i2c_reg_array)),
                GFP_KERNEL);
    ...
    if (copy_from_user(stop_setting->reg_setting, (void *)reg_setting,
                       stop_setting->size *
                           sizeof(struct msm_camera_i2c_reg_array))) {
      pr_err("%s:%d failed\n", __func__, __LINE__);
...
int msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp) {
  ...
  case CFG_SLAVE_READ_I2C: {
    ...
    read_config_ptr = (struct msm_camera_i2c_read_config *)cdata->cfg.setting;
    if (copy_from_user(&read_config, read_config_ptr,
                       sizeof(struct msm_camera_i2c_read_config))) {
      pr_err("%s:%d failed\n", __func__, __LINE__);
      rc = -EFAULT;
  ...
  case CFG_SLAVE_WRITE_I2C_ARRAY: {
    ...
    if (copy_from_user(&write_config, (void *)cdata->cfg.setting,
    ...
    CDBG("%s:slave_addr=0x%x, array_size=%d\n", __func__,
         write_config.slave_addr, write_config.conf_array.size);
    ...
    if (s_ctrl->sensor_i2c_client->cci_client) {
      orig_slave_addr = s_ctrl->sensor_i2c_client->cci_client->sid;
      s_ctrl->sensor_i2c_client->cci_client->sid = write_slave_addr >> 1;
    } else if (s_ctrl->sensor_i2c_client->client) {
      orig_slave_addr = s_ctrl->sensor_i2c_client->client->addr;
      s_ctrl->sensor_i2c_client->client->addr = write_slave_addr >> 1;
    } else {
      pr_err("%s: error: no i2c/cci client found.", __func__);
      kfree(reg_setting);
      rc = -EFAULT;
    ...
    CDBG("%s:orig_slave_addr=0x%x, new_slave_addr=0x%x", __func__,
         orig_slave_addr, write_slave_addr >> 1);
    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_table(
...
static int msm_sensor_v4l2_enum_fmt(struct v4l2_subdev *sd, unsigned int index,
...
int32_t msm_sensor_platform_probe(struct platform_device *pdev,
  ...
  if (!s_ctrl->sensor_i2c_client->i2c_func_tbl)
    s_ctrl->sensor_i2c_client->i2c_func_tbl = &msm_sensor_cci_func_tbl;
   ...
   * Boot-time power_up/down reboots cityman (kernels smia-msm #1-#3).
   */
...
int msm_sensor_i2c_probe(struct i2c_client *client,
  ...
  if (!s_ctrl->func_tbl)
    s_ctrl->func_tbl = &msm_sensor_func_tbl;
  if (!s_ctrl->sensor_i2c_client->i2c_func_tbl)
    s_ctrl->sensor_i2c_client->i2c_func_tbl = &msm_sensor_qup_func_tbl;
  if (!s_ctrl->sensor_v4l2_subdev_ops)
    s_ctrl->sensor_v4l2_subdev_ops = &msm_sensor_subdev_ops;
  ...
  if (!client->dev.of_node) {
    ...
    memcpy(s_ctrl->sensordata->power_info.clk_info, cam_8960_clk_info,
    ...
    memcpy(s_ctrl->sensordata->power_info.clk_info, cam_8610_clk_info,
  ...
  rc = camera_init_v4l2(&s_ctrl->sensor_i2c_client->client->dev, &session_id);
  CDBG("%s rc %d session_id %d\n", __func__, rc, session_id);
  s_ctrl->sensordata->sensor_info->session_id = session_id;
  s_ctrl->msm_sd.close_seq = MSM_SD_CLOSE_2ND_CATEGORY | 0x3;
  msm_sd_register(&s_ctrl->msm_sd);
...
int32_t msm_sensor_init_default_params(struct msm_sensor_ctrl_t *s_ctrl) {
  ...
  if (!s_ctrl->sensor_i2c_client) {
    pr_err("%s:%d failed: invalid params sensor_i2c_client %pK\n", __func__,
           __LINE__, s_ctrl->sensor_i2c_client);
    return -EINVAL;
  }
