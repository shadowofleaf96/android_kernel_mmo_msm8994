...
int msm_sensor_driver_bind_probed(struct msm_sensor_ctrl_t *s_ctrl,
  ...
  if (s_ctrl->id >= MAX_CAMERAS) {
    pr_err("cityman_smia: bind slot id %u out of range\n", s_ctrl->id);
    return -EINVAL;
  }
  if (g_sctrl[s_ctrl->id] && g_sctrl[s_ctrl->id] != s_ctrl) {
    pr_err("cityman_smia: g_sctrl[%u] already taken\n", s_ctrl->id);
    return -EBUSY;
  }
  ...
  pr_err("cityman_smia: g_sctrl[%u]=%s id=0x%04x (CFG_SINIT_PROBE slot)\n",
...
static int32_t
msm_sensor_driver_create_i2c_v4l_subdev(struct msm_sensor_ctrl_t *s_ctrl) {
  int32_t rc = 0;
...
static int32_t
msm_sensor_driver_create_v4l_subdev(struct msm_sensor_ctrl_t *s_ctrl) {
  int32_t rc = 0;
  uint32_t session_id = 0;
...
static int32_t
msm_sensor_fill_eeprom_subdevid_by_name(struct msm_sensor_ctrl_t *s_ctrl) {
  int32_t rc = 0;
  ...
  *eeprom_subdev_id = -1;
  ...
  for (i = 0; i < count; i++) {
    ...
    rc = of_property_read_string(src_node, "qcom,eeprom-name", &eeprom_name);
    if (rc < 0) {
      pr_err("failed\n");
...
static int32_t
msm_sensor_fill_actuator_subdevid_by_name(struct msm_sensor_ctrl_t *s_ctrl) {
  int32_t rc = 0;
  struct device_node *src_node = NULL;
...
static int32_t
msm_sensor_fill_ois_subdevid_by_name(struct msm_sensor_ctrl_t *s_ctrl) {
  int32_t rc = 0;
  struct device_node *src_node = NULL;
...
static int32_t msm_sensor_fill_slave_info_init_params(
  ...
  if (INVALID_CAMERA_B != sensor_init_params->position)
    sensor_info->position = sensor_init_params->position;
  ...
  if (CAMERA_MODE_INVALID != sensor_init_params->modes_supported)
    sensor_info->modes_supported = sensor_init_params->modes_supported;

  return 0;
}

static int32_t
msm_sensor_validate_slave_info(struct msm_sensor_info_t *sensor_info) {
...
#ifdef CONFIG_COMPAT
static int32_t
msm_sensor_get_pw_settings_compat(struct msm_sensor_power_setting *ps,
                                  struct msm_sensor_power_setting *us_ps,
                                  uint32_t size) {
  int32_t rc = 0, i = 0;
...
static int32_t msm_sensor_create_pd_settings(
    void *setting, struct msm_sensor_power_setting *pd, uint32_t size_down,
...
#ifdef CONFIG_COMPAT
  if (is_compat_task()) {
    rc = msm_sensor_get_pw_settings_compat(pd, pu, size_down);
    if (rc < 0) {
      pr_err("failed");
      return -EFAULT;
    }
...
static int32_t msm_sensor_get_power_down_settings(
    void *setting, struct msm_camera_sensor_slave_info *slave_info,
  ...
  if (slave_info->power_setting_array.power_down_setting) {
    ...
    if (is_compat_task()) {
      rc = msm_sensor_get_pw_settings_compat(
          pd, slave_info->power_setting_array.power_down_setting, size_down);
      if (rc < 0) {
        pr_err("failed");
        kfree(pd);
        return -EFAULT;
...
#endif
        if (copy_from_user(
                pd, (void *)slave_info->power_setting_array.power_down_setting,
                sizeof(*pd) * size_down)) {
      pr_err("failed: copy_from_user");
      kfree(pd);
      return -EFAULT;
    ...
    rc = msm_sensor_create_pd_settings(
        setting, pd, size_down, slave_info->power_setting_array.power_setting);
    if (rc < 0) {
      pr_err("failed");
      kfree(pd);
      return -EFAULT;
  ...
  for (i = 0; i < size_down; i++) {
    CDBG("DOWN seq_type %d seq_val %d config_val %ld delay %d", pd[i].seq_type,
         pd[i].seq_val, pd[i].config_val, pd[i].delay);
  }
  return rc;
}

static int32_t msm_sensor_get_power_up_settings(
    void *setting, struct msm_camera_sensor_slave_info *slave_info,
    struct msm_camera_power_ctrl_t *power_info) {
  int32_t rc = 0;
  uint16_t size = 0;
  ...
  if (is_compat_task()) {
    rc = msm_sensor_get_pw_settings_compat(
        pu, slave_info->power_setting_array.power_setting, size);
    if (rc < 0) {
      pr_err("failed");
      kfree(pu);
      return -EFAULT;
  ...
  /* Print power setting */
  for (i = 0; i < size; i++) {
    CDBG("UP seq_type %d seq_val %d config_val %ld delay %d", pu[i].seq_type,
         pu[i].seq_val, pu[i].config_val, pu[i].delay);
  }
...
static int32_t
msm_sensor_get_power_settings(void *setting,
                              struct msm_camera_sensor_slave_info *slave_info,
                              struct msm_camera_power_ctrl_t *power_info) {
  int32_t rc = 0;
...
static void msm_sensor_fill_sensor_info(struct msm_sensor_ctrl_t *s_ctrl,
  ...
  for (i = 0; i < SUB_MODULE_MAX; i++) {
    sensor_info->subdev_id[i] = s_ctrl->sensordata->sensor_info->subdev_id[i];
  ...
  sensor_info->position = s_ctrl->sensordata->sensor_info->position;
...
int32_t msm_sensor_driver_probe(void *setting,
  ...
  if (is_compat_task()) {
    ...
    if (copy_from_user((void *)slave_info32, setting, sizeof(*slave_info32))) {
      pr_err("failed: copy_from_user");
      rc = -EFAULT;
      kfree(slave_info32);
      goto free_slave_info;
    ...
    slave_info->power_setting_array.power_setting =
        compat_ptr(slave_info32->power_setting_array.power_setting);
    slave_info->power_setting_array.power_down_setting =
        compat_ptr(slave_info32->power_setting_array.power_down_setting);
    slave_info->is_init_params_valid = slave_info32->is_init_params_valid;
    slave_info->sensor_init_params = slave_info32->sensor_init_params;
    slave_info->is_flash_supported = slave_info32->is_flash_supported;
    kfree(slave_info32);
  } else
#endif
  {
  ...
  CDBG("camera id %d Slave addr 0x%X addr_type %d\n", slave_info->camera_id,
       slave_info->slave_addr, slave_info->addr_type);
  ...
  if (s_ctrl->is_probe_succeed == 1) {
     ...
     * probe
     */
    if (1 || slave_info->sensor_id_info.sensor_id ==
        s_ctrl->sensordata->cam_slave_info->sensor_id_info.sensor_id) {
      pr_err("slot%d: sensor id%d already probed (spoofed)\n", slave_info->camera_id,
             s_ctrl->sensordata->cam_slave_info->sensor_id_info.sensor_id);
  ...
  if (!s_ctrl->sensor_i2c_client) {
    pr_err("failed: sensor_i2c_client %pK", s_ctrl->sensor_i2c_client);
    rc = -EINVAL;
  ...
  if (rc < 0) {
    pr_err("failed: msm_camera_get_dt_power_setting_data rc %d", rc);
    goto free_camera_info;
  }
  ...
  if (slave_info->is_flash_supported == 0) {
    s_ctrl->sensordata->sensor_info->subdev_id[SUB_MODULE_LED_FLASH] = -1;
  }
  ...
  rc = msm_sensor_fill_slave_info_init_params(slave_info,
  ...
  if (rc < 0) {
    pr_err("%s Validate slave info failed", slave_info->sensor_name);
    goto free_camera_info;
  }
...
static int32_t
msm_sensor_driver_get_gpio_data(struct msm_camera_sensor_board_info *sensordata,
  ...
  if (!sensordata || !of_node) {
    pr_err("failed: invalid params sensordata %pK of_node %pK", sensordata,
           of_node);
    return -EINVAL;
  }
  ...
  rc =
      msm_camera_init_gpio_pin_tbl(of_node, gconf, gpio_array, gpio_array_size);
  if (rc < 0) {
    pr_err("failed");
...
static int32_t msm_sensor_driver_parse(struct msm_sensor_ctrl_t *s_ctrl) {
  ...
  s_ctrl->sensor_i2c_client =
      kzalloc(sizeof(*s_ctrl->sensor_i2c_client), GFP_KERNEL);
  if (!s_ctrl->sensor_i2c_client) {
  ...
  s_ctrl->msm_sensor_mutex =
      kzalloc(sizeof(*s_ctrl->msm_sensor_mutex), GFP_KERNEL);
  if (!s_ctrl->msm_sensor_mutex) {
    pr_err("failed: no memory msm_sensor_mutex %pK", s_ctrl->msm_sensor_mutex);
