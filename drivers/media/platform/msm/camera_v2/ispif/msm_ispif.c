...
static struct msm_cam_clk_info ispif_8626_reset_clk_info[] = {
    {"ispif_ahb_clk", NO_SET_RATE},     {"camss_top_ahb_clk", NO_SET_RATE},
    {"csi0_ahb_clk", NO_SET_RATE},      {"csi0_src_clk", NO_SET_RATE},
    {"csi0_phy_clk", NO_SET_RATE},      {"csi0_clk", NO_SET_RATE},
    {"csi0_pix_clk", NO_SET_RATE},      {"csi0_rdi_clk", NO_SET_RATE},
    {"csi1_ahb_clk", NO_SET_RATE},      {"csi1_src_clk", NO_SET_RATE},
    {"csi1_phy_clk", NO_SET_RATE},      {"csi1_clk", NO_SET_RATE},
    {"csi1_pix_clk", NO_SET_RATE},      {"csi1_rdi_clk", NO_SET_RATE},
    {"camss_vfe_vfe_clk", NO_SET_RATE}, {"camss_csi_vfe_clk", NO_SET_RATE},
};
...
static int msm_ispif_reset_hw(struct ispif_device *ispif) {
  ...
  rc = msm_ispif_get_clk_info(ispif, ispif->pdev, ispif_ahb_clk_info,
                              ispif_clk_info);
  if (rc < 0) {
  ...
  rc = msm_cam_clk_enable(&ispif->pdev->dev, ispif_clk_info, ispif->clk,
                          ispif->num_clk, 1);
  if (rc < 0) {
    pr_err("%s: cannot enable clock, error = %d\n", __func__, rc);
    rc = msm_cam_clk_enable(&ispif->pdev->dev, ispif_8626_reset_clk_info,
                            reset_clk1, ARRAY_SIZE(ispif_8626_reset_clk_info),
                            1);
    if (rc < 0) {
      pr_err("%s: cannot enable clock, error = %d", __func__, rc);
    } else {
  ...
  timeout = wait_for_completion_timeout(&ispif->reset_complete[VFE0],
  ...
  if (ispif->hw_num_isps > 1) {
    msm_camera_io_w(ISPIF_RST_CMD_1_MASK, ispif->base + ISPIF_RST_CMD_1_ADDR);
    timeout = wait_for_completion_timeout(&ispif->reset_complete[VFE1],
                                          msecs_to_jiffies(500));
    CDBG("%s: VFE1 done\n", __func__);
    if (timeout <= 0) {
  ...
  if (ispif->clk_idx == 1) {
    rc = msm_cam_clk_enable(&ispif->pdev->dev, ispif_clk_info, ispif->clk,
                            ispif->num_clk, 0);
    if (rc < 0) {
      pr_err("%s: cannot disable clock, error = %d", __func__, rc);
    }
  }
...
int msm_ispif_get_clk_info(struct ispif_device *ispif_dev,
  ...
  for (i = 0; i < count; i++) {
    ...
    rc = of_property_read_string_index(of_node, "qcom,clock-rates", i, &rate);
    ...
    if (strnstr(clk_info[i].clk_name, "ahb", strlen(clk_info[i].clk_name))) {
      ahb_clk_info[num_ahb_clk].clk_name = clk_info[i].clk_name;
      ahb_clk_info[num_ahb_clk].clk_rate = clk_info[i].clk_rate;
...
static int msm_ispif_clk_ahb_enable(struct ispif_device *ispif, int enable) {
  ...
  rc = msm_cam_clk_enable(&ispif->pdev->dev, ispif_ahb_clk_info, ispif->ahb_clk,
...
static int msm_ispif_reset(struct ispif_device *ispif) {
  ...
  for (i = 0; i < ispif->vfe_info.num_vfe; i++) {
    ...
    msm_camera_io_w(0xFFFFFFFF, ispif->base + ISPIF_VFE_m_IRQ_CLEAR_0(i));
    msm_camera_io_w(0xFFFFFFFF, ispif->base + ISPIF_VFE_m_IRQ_CLEAR_1(i));
    msm_camera_io_w(0xFFFFFFFF, ispif->base + ISPIF_VFE_m_IRQ_CLEAR_2(i));
    ...
    msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_PIX_INTF_n_CID_MASK(i, 0));
    msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_PIX_INTF_n_CID_MASK(i, 1));
    msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_RDI_INTF_n_CID_MASK(i, 0));
    msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_RDI_INTF_n_CID_MASK(i, 1));
    msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_RDI_INTF_n_CID_MASK(i, 2));

    msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_PIX_INTF_n_CROP(i, 0));
    msm_camera_io_w(0, ispif->base + ISPIF_VFE_m_PIX_INTF_n_CROP(i, 1));
  }
...
static void msm_ispif_sel_csid_core(struct ispif_device *ispif,
  ...
  msm_camera_io_w_mb(data, ispif->base + ISPIF_VFE_m_INPUT_SEL(vfe_intf));
}

static void msm_ispif_enable_crop(struct ispif_device *ispif, uint8_t intftype,
                                  enum msm_ispif_vfe_intf vfe_intf,
  ...
  msm_camera_io_w(data, ispif->base + ISPIF_VFE_m_CTRL_0(vfe_intf));
...
static void msm_ispif_select_clk_mux(struct ispif_device *ispif,
  ...
  case RDI0:
    data = msm_camera_io_r(ispif->clk_mux_base + ISPIF_RDI_CLK_MUX_SEL_ADDR);
    data &= ~(0xf << (vfe_intf * 12));
    data |= (csid << (vfe_intf * 12));
    msm_camera_io_w(data, ispif->clk_mux_base + ISPIF_RDI_CLK_MUX_SEL_ADDR);
    break;
...
static uint16_t
msm_ispif_get_cids_mask_from_cfg(struct msm_ispif_params_entry *entry) {
  int i;
...
static int msm_ispif_config(struct ispif_device *ispif,
  ...
  for (i = 0; i < params->num; i++) {
    ...
    if (intftype >= RDI0)
      pr_err("%s intftype %x, vfe_intf %d, csid %d\n", __func__, intftype,
             vfe_intf, params->entries[i].csid);

    if ((intftype >= INTF_MAX) || (vfe_intf >= ispif->vfe_info.num_vfe) ||
        (ispif->csid_version <= CSID_VERSION_V22 && (vfe_intf > VFE0))) {
    ...
    if (ispif->csid_version >= CSID_VERSION_V30)
      msm_ispif_select_clk_mux(ispif, intftype, params->entries[i].csid,
                               vfe_intf);
  ...
  for (vfe_intf = 0; vfe_intf < 2; vfe_intf++) {
    msm_camera_io_w(ISPIF_IRQ_STATUS_MASK,
                    ispif->base + ISPIF_VFE_m_IRQ_MASK_0(vfe_intf));

    msm_camera_io_w(ISPIF_IRQ_STATUS_MASK,
                    ispif->base + ISPIF_VFE_m_IRQ_CLEAR_0(vfe_intf));

    msm_camera_io_w(ISPIF_IRQ_STATUS_1_MASK,
                    ispif->base + ISPIF_VFE_m_IRQ_MASK_1(vfe_intf));

    msm_camera_io_w(ISPIF_IRQ_STATUS_1_MASK,
                    ispif->base + ISPIF_VFE_m_IRQ_CLEAR_1(vfe_intf));

    msm_camera_io_w(ISPIF_IRQ_STATUS_2_MASK,
                    ispif->base + ISPIF_VFE_m_IRQ_MASK_2(vfe_intf));

    msm_camera_io_w(ISPIF_IRQ_STATUS_2_MASK,
                    ispif->base + ISPIF_VFE_m_IRQ_CLEAR_2(vfe_intf));
  }
...
static void msm_ispif_intf_cmd(struct ispif_device *ispif, uint32_t cmd_bits,
  ...
  for (i = 0; i < params->num; i++) {
    ...
    for (k = 0; k < params->entries[i].num_cids; k++) {
      ...
      if (intf_type == RDI2) {
        /* zero out two bits */
        ispif->applied_intf_cmd[vfe_intf].intf_cmd1 &= ~(0x3 << (vc * 2 + 8));
        /* set cmd bits */
...
static int msm_ispif_stop_immediately(struct ispif_device *ispif,
  ...
  for (i = 0; i < params->num; i++) {
    cid_mask = msm_ispif_get_cids_mask_from_cfg(&params->entries[i]);
    msm_ispif_enable_intf_cids(ispif, params->entries[i].intftype, cid_mask,
...
static int
msm_ispif_restart_frame_boundary(struct ispif_device *ispif,
                                 struct msm_ispif_param_data *params) {
  int rc = 0, i;
  long timeout = 0;
  uint16_t cid_mask;
  ...
  if (vfe_mask & (1 << VFE0)) {
    ...
    /* initiate reset of ISPIF */
    msm_camera_io_w(0x00001FF9, ispif->base + ISPIF_RST_CMD_ADDR);
  }
  if (ispif->hw_num_isps > 1 && (vfe_mask & (1 << VFE1))) {
    init_completion(&ispif->reset_complete[VFE1]);
    pr_err("%s Init completion VFE1\n", __func__);
    msm_camera_io_w(0x00001FF9, ispif->base + ISPIF_RST_CMD_1_ADDR);
  }
...
static int msm_ispif_stop_frame_boundary(struct ispif_device *ispif,
  ...
  msm_ispif_intf_cmd(ispif, ISPIF_INTF_CMD_DISABLE_FRAME_BOUNDARY, params);
  ...
  for (i = 0; i < params->num; i++) {
    ...
    rc = readl_poll_timeout(ispif->base + intf_addr, stop_flag,
                            (stop_flag & 0xF) == 0xF, ISPIF_TIMEOUT_SLEEP_US,
...
static void ispif_process_irq(struct ispif_device *ispif,
  ...
  if (out[vfe_id].ispifIrqStatus0 & ISPIF_IRQ_STATUS_PIX_SOF_MASK) {
    ispif->sof_count[vfe_id].sof_cnt[PIX0]++;
    pr_err_ratelimited("cityman_ispif PIX SOF vfe=%d n=%u st0=0x%x\n", vfe_id,
                       ispif->sof_count[vfe_id].sof_cnt[PIX0],
                       out[vfe_id].ispifIrqStatus0);
  }
  if (out[vfe_id].ispifIrqStatus0 & ISPIF_IRQ_STATUS_RDI0_SOF_MASK) {
    ispif->sof_count[vfe_id].sof_cnt[RDI0]++;
    pr_err_ratelimited("cityman_ispif RDI0 SOF vfe=%d n=%u st0=0x%x\n", vfe_id,
                       ispif->sof_count[vfe_id].sof_cnt[RDI0],
                       out[vfe_id].ispifIrqStatus0);
  }
  if (out[vfe_id].ispifIrqStatus1 & ISPIF_IRQ_STATUS_RDI1_SOF_MASK) {
    ispif->sof_count[vfe_id].sof_cnt[RDI1]++;
  }
  if (out[vfe_id].ispifIrqStatus2 & ISPIF_IRQ_STATUS_RDI2_SOF_MASK) {
...
static int msm_ispif_init(struct ispif_device *ispif, uint32_t csid_version) {
  int rc = 0;
  ...
  if (ispif->csid_version >= CSID_VERSION_V30) {
    if (!ispif->clk_mux_mem || !ispif->clk_mux_io) {
      pr_err("%s csi clk mux mem %pK io %pK\n", __func__, ispif->clk_mux_mem,
             ispif->clk_mux_io);
      rc = -ENOMEM;
      return rc;
    }
    ispif->clk_mux_base =
        ioremap(ispif->clk_mux_mem->start, resource_size(ispif->clk_mux_mem));
  ...
  rc = request_irq(ispif->irq->start, msm_io_ispif_irq, IRQF_TRIGGER_RISING,
                   "ispif", ispif);
  if (rc) {
...
static long msm_ispif_cmd(struct v4l2_subdev *sd, void *arg) {
  ...
  struct ispif_device *ispif = (struct ispif_device *)v4l2_get_subdevdata(sd);
...
static long msm_ispif_subdev_do_ioctl(struct file *file, unsigned int cmd,
                                      void *arg) {
...
static int ispif_probe(struct platform_device *pdev) {
  ...
  if (pdev->dev.of_node) {
    ...
    rc = of_property_read_u32((&pdev->dev)->of_node, "qcom,num-isps",
                              &ispif->hw_num_isps);
    if (rc)
  ...
  rc = msm_ispif_get_clk_info(ispif, pdev, ispif_ahb_clk_info, ispif_clk_info);
  if (rc < 0) {
    pr_err("%s: msm_isp_get_clk_info() failed", __func__);
    return -EFAULT;
  }
  ...
  ispif->mem = platform_get_resource_byname(pdev, IORESOURCE_MEM, "ispif");
  if (!ispif->mem) {
    pr_err("%s: no mem resource?\n", __func__);
    rc = -ENODEV;
    goto error;
  }
  ispif->irq = platform_get_resource_byname(pdev, IORESOURCE_IRQ, "ispif");
  if (!ispif->irq) {
    pr_err("%s: no irq resource?\n", __func__);
    rc = -ENODEV;
    goto error;
  }
  ispif->io = request_mem_region(ispif->mem->start, resource_size(ispif->mem),
                                 pdev->name);
  if (!ispif->io) {
  ...
  ispif->clk_mux_mem =
      platform_get_resource_byname(pdev, IORESOURCE_MEM, "csi_clk_mux");
  if (ispif->clk_mux_mem) {
    ispif->clk_mux_io = request_mem_region(ispif->clk_mux_mem->start,
  ...
  snprintf(ispif->msm_sd.sd.name, ARRAY_SIZE(ispif->msm_sd.sd.name),
           MSM_ISPIF_DRV_NAME);
